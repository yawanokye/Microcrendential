...
export async function getIdentityAndProfile() {
  ...
  const profile = await db.prepare("SELECT email, full_name, role, status, identity_status, date_of_birth, gender, nationality, phone, address, id_type, id_last4 FROM users WHERE email = ? LIMIT 1")
