...
import {
  ...
  Code2, FileCheck2, FileText, FlaskConical, Gauge, GraduationCap, GripVertical, HeartPulse, ImageIcon, LayoutDashboard, Menu,
...
export default function Home() {
  ...
  useEffect(() => { setInviteToken(new URLSearchParams(window.location.search).get("invite") ?? ""); fetch("/api/auth/session").then((response) => response.json()).then((data: AccountSession) => setAccount(data)).catch(() => setAccount({ authenticated: false })); }, []);
...
function PortalAccess({ inviteToken }: { inviteToken: string }) {
  const returnTo = inviteToken ? `/?invite=${encodeURIComponent(inviteToken)}` : "/";
  return <main className="access-shell"><section className="access-card"><div className="access-brand"><div className="brand-mark"><GraduationCap size={28} /></div><div><strong>UCC Microcredentials</strong><span>University of Cape Coast</span></div></div><p className="eyebrow">SECURE PORTAL ACCESS</p><h1>{inviteToken ? "Complete your facilitator invitation." : "Learn, facilitate or administer from one platform."}</h1><p>{inviteToken ? "Sign in with the exact email address invited by the UCC administrator. You will then complete permanent biodata and identity verification." : "Sign in to continue. New learners submit biodata, national ID evidence and a live selfie. Facilitator access begins with an administrator-issued invitation."}</p><div className="portal-role-grid"><article><BookOpen /><b>Learner</b><span>Self-register and complete identity review</span></article><article><ShieldCheck /><b>Facilitator</b><span>Accept an administrator-issued setup link</span></article><article><Users /><b>System admin</b><span>Compare identity evidence and activate access</span></article></div><a className="access-primary" href={`/signin-with-chatgpt?return_to=${encodeURIComponent(returnTo)}`} target="_top"><ShieldCheck size={18} /> Sign in to UCC Microcredentials</a><small>Sign-in identifies the email owner; UCC verification controls portal activation.</small></section></main>;
}
...
function VirtualPracticalRunner({ practical, latest, onSubmitted }: { practical: VirtualPractical; latest?: VirtualLabSubmission; onSubmitted: () => Promise<void> }) {
  const initialStage = latest?.status === "submitted" ? 8 : latest?.status === "assessed" ? 9 : 1;
  ...
  const [observations, setObservations] = useState<{ trial: string; input: number; result: number; note: string }[]>([]);
  ...
  return <div className="practical-runner"><div className="lab-disclaimer"><ShieldCheck /><div><b>Complementary virtual practice</b><p>This simulation supports preparation, repetition and assessment. It does not replace required physical laboratory work, clinical placement, supervision or regulatory competency assessment.</p></div></div><div className="lab-stage-track">{["Brief", "Safety", "Equipment", "Simulate", "Checkpoint", "Data", "Report", "Grade", "Debrief"].map((label, index) => <span key={label} className={stage === index + 1 ? "active" : stage > index + 1 ? "complete" : ""}><b>{stage > index + 1 ? "✓" : index + 1}</b>{label}</span>)}</div>
    ...
    {stage === 3 && <section className="lab-stage"><p className="eyebrow">EQUIPMENT IDENTIFICATION</p><h3>Select every item required for this virtual activity</h3><div className="equipment-grid">{practical.equipment.map((item) => <button key={item} className={equipment.includes(item) ? "selected" : ""} onClick={() => toggleEquipment(item)}><VirtualLabIcon discipline={practical.discipline} /><span>{item}</span>{equipment.includes(item) && <CheckCircle2 />}</button>)}</div><button className="dialog-primary" disabled={equipment.length !== practical.equipment.length} onClick={() => setStage(4)}>Open interactive simulation <ChevronRight /></button></section>}
    {stage === 4 && <section className="lab-stage"><p className="eyebrow">INTERACTIVE SIMULATION</p><h3>{practical.mode === "measurement" ? "Change the input and record the model response" : "Complete the safest sequence"}</h3>{practical.mode === "measurement" ? <div className="virtual-instrument"><div className="instrument-screen"><Activity /><span>{practical.resultLabel}</span><strong>{result} {practical.resultUnit}</strong></div><label>{practical.parameterLabel}: <b>{parameter} {practical.parameterUnit}</b><input type="range" min={practical.parameterMin} max={practical.parameterMax} value={parameter} onChange={(event) => setParameter(Number(event.target.value))} /></label><label>Observation note<input value={note} onChange={(event) => setNote(event.target.value)} placeholder="What changed and what did you notice?" /></label><button className="secondary-action" onClick={runMeasurement}><Gauge /> Run and record trial</button></div> : <div className="procedure-simulator"><div className="virtual-patient"><VirtualLabIcon discipline={practical.discipline} /><span>Fictional supervised scenario</span><b>{Math.round((sequenceIndex / practical.procedureSteps.length) * 100)}% complete</b></div><p>Select the next safest step:</p><div>{orderedChoices.filter((item) => !practical.procedureSteps.slice(0, sequenceIndex).includes(item)).map((item) => <button key={item} onClick={() => chooseProcedureStep(item)}>{item}<ChevronRight /></button>)}</div></div>}<button className="dialog-primary" disabled={!ran} onClick={() => setStage(5)}>Pause and assess understanding <FileCheck2 /></button></section>}
    ...
    {stage === 9 && <section className="lab-stage lab-debrief"><CheckCircle2 /><h3>{latest?.passed ? "Competency evidence accepted" : "Assessment completed"}</h3><div className="lab-grade"><strong>{latest?.mark ?? 0}%</strong><span>{latest?.passed ? "Competent in this virtual activity" : "Further development required"}</span></div><div className="lab-feedback"><b>Facilitator feedback</b><p>{latest?.feedback || "No feedback recorded."}</p><b>Competency note</b><p>{latest?.competencyNote || "No competency note recorded."}</p></div><details open><summary>Debriefing guidance</summary><p>{practical.debrief}</p></details><p className="lab-notice">Completion of this virtual activity does not certify independent physical or clinical competence.</p>{!latest?.passed && <button className="secondary-action" onClick={reset}><RotateCcw /> Repeat virtual practical</button>}</section>}
  </div>;
}
...
function VirtualLabsWorkspace({ role }: { role: PortalRole }) {
  ...
  if (role !== "learner") return <div className="virtual-lab-admin"><section className="page-panel"><div className="page-title"><div><p className="eyebrow">VIRTUAL PRACTICALS</p><h2>Simulation laboratory catalogue</h2><p>{virtualPracticals.length} complementary practicals across science, health and engineering. These activities do not replace required real laboratory or clinical work.</p></div><span className="access-badge"><FlaskConical /> Facilitator supervised</span></div><div className="lab-discipline-summary">{labDisciplines.map((item) => <button key={item} onClick={() => { setDiscipline(item); setSelected(virtualPracticals.find((practical) => practical.discipline === item) ?? null); }}><VirtualLabIcon discipline={item} /><span><b>{item}</b>{virtualPracticals.filter((practical) => practical.discipline === item).length} practicals</span><ChevronRight /></button>)}</div></section><section className="page-panel"><div className="page-title"><div><p className="eyebrow">PRACTICAL ASSESSMENT REGISTER</p><h2>Learner evidence and competency decisions</h2><p>Grade the virtual evidence while clearly recording what still requires physical supervision.</p></div><span className="access-badge"><ClipboardCheck /> {submissions.filter((item) => item.status === "submitted").length} awaiting review</span></div>{loading && <div className="empty-state">Loading practical submissions…</div>}<div className="lab-review-list">{submissions.map((submission) => <VirtualLabSubmissionReview key={submission.id} submission={submission} onUpdated={load} />)}{!loading && submissions.length === 0 && <div className="empty-state">Learner practical submissions will appear here.</div>}</div></section></div>;
