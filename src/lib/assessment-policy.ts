...
export type AssessmentQuestionRecord = { id: string; type: string; prompt?: string; options?: string[]; correctAnswer?: string; points?: number; scheme?: string; outcomeIds?: string[]; pairs?: AssessmentPair[]; videoUrl?: string; videoMode?: "whole" | "part" | "pause"; videoStart?: number; videoEnd?: number; whiteboardEnabled?: boolean; previewed?: boolean; approved?: boolean };
...
export function learnerSafeAssessmentConfig(config: AssessmentConfigRecord, limit = 100) {
  const approved = (config.questions ?? []).filter((q) => q.approved !== false).slice(0, limit);
  return { passMark: Math.min(100, Math.max(1, Number(config.passMark) || 70)), attempts: String(allowedAttempts(config)), questions: approved.map((q) => ({ id: q.id, type: q.type, prompt: q.prompt ?? "", options: q.options ?? [], points: Math.max(1, Number(q.points) || 1), outcomeIds: q.outcomeIds ?? [], pairs: (q.pairs ?? []).map((pair) => ({ left: pair.left, image: pair.image })), pairOptions: [...new Set((q.pairs ?? []).map((pair) => pair.right).filter(Boolean))].reverse(), videoUrl: q.videoUrl, videoMode: q.videoMode, videoStart: q.videoStart, videoEnd: q.videoEnd, whiteboardEnabled: q.whiteboardEnabled, markingMode: requiresHumanMarking(q) ? "human" : "automatic" })) };
}
...
export function validateAssessmentForPublication(config: AssessmentConfigRecord, limit = 100) {
  ...
  const questions=allQuestions.filter((q)=>q.approved !== false);
  const issues:string[]=[];
