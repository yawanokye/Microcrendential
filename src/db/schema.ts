...
export const courseDrafts = sqliteTable("course_drafts", {
  ...
  certificatePreapproved: integer("certificate_preapproved", { mode: "boolean" }).notNull().default(false),
  approvalAuthority: text("approval_authority"),
  approvalMeetingDate: text("approval_meeting_date"),
  approvalReference: text("approval_reference"),
  approvalDecision: text("approval_decision"),
  approvalConditions: text("approval_conditions"),
  approvalConditionsSatisfied: integer("approval_conditions_satisfied", { mode: "boolean" }).notNull().default(false),
  approvalEffectiveDate: text("approval_effective_date"),
  approvalReviewDate: text("approval_review_date"),
  approvalDocumentReference: text("approval_document_reference"),
  approvalRecordedByEmail: text("approval_recorded_by_email"),
  approvalRecordedAt: text("approval_recorded_at"),
...
export const paymentOrders = sqliteTable("payment_orders", {
  ...
  status: text("status", { enum: ["initialized", "paid", "failed", "cancelled"] }).notNull().default("initialized"),
  providerDataJson: text("provider_data_json").notNull().default("{}"),
  paidAt: text("paid_at"),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
  uniqueIndex("payment_orders_reference_unique").on(table.reference),
  index("payment_orders_user_idx").on(table.userEmail),
  index("payment_orders_course_idx").on(table.courseCode),
]);
