...
export default function AuthForm({ returnTo, portal: requestedPortal, inviteToken = "" }: AuthFormProps) {
  ...
  return <main className={`access-shell render-auth-shell commercial-signin ${portal}`}><section className="commercial-signin-card"><aside><a className="signin-brand" href="/"><GraduationCap /><div><b className="ucc-growth-wordmark">UCC <span>Growth<sup>+</sup></span></b><span>University of Cape Coast</span></div></a><div className="signin-value"><p className="eyebrow">{portalName.toUpperCase()}</p><h1>{heading}</h1><p>{description}</p></div><div className="signin-signals"><span><ShieldCheck /><b>Role-bound gateway</b><small>Only the matching account role can establish a session here.</small></span><span><GraduationCap /><b>Verified achievement</b><small>Learning evidence and credential decisions remain governed.</small></span></div><small>Secure session · Salted password hash · Identity-controlled profile</small></aside><section><a className="signin-back" href="/"><ArrowLeft /> Public website</a><div className={`auth-portal-badge ${portal}`}><PortalIcon /><span>{portalName.toUpperCase()}</span></div><h2>{mode === "admin_setup" ? "Initial administrator setup" : mode === "facilitator_setup" ? "Invited facilitator activation" : "Sign in securely"}</h2><p>{mode === "admin_setup" ? "Create the permanent credentials for the configured administrator email." : mode === "facilitator_setup" ? "This gateway validates both the invited email and its one-time invitation." : `Enter the credentials associated with your ${portalName} account.`}</p>{isAdmin && <div className="auth-mode"><button type="button" className={mode === "login" ? "active" : ""} onClick={() => { setMode("login"); setError(""); }}>Admin sign in</button><button type="button" className={mode === "admin_setup" ? "active" : ""} onClick={() => { setMode("admin_setup"); setError(""); }}>First admin setup</button></div>}<form className="render-auth-form" onSubmit={submit}>{mode === "admin_setup" && <label>Full legal name<input value={fullName} onChange={(event) => setFullName(event.target.value)} autoComplete="name" required /></label>}<label>{isAdmin ? "Administrator email" : isFacilitator ? "Institutional email" : "Learner email"}<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" required /></label><label>{isSetup ? "Create or confirm password" : "Password"}<input type="password" minLength={10} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete={isSetup ? "new-password" : "current-password"} required /><span>{isSetup ? "Use at least 10 characters." : "Minimum 10 characters"}</span></label>{error && <div className="auth-error" role="alert">{error}</div>}<button className={`access-primary ${isAdmin ? "admin-submit" : ""}`} disabled={loading}><PortalIcon /> {loading ? "Verifying access…" : mode === "admin_setup" ? "Create administrator access" : mode === "facilitator_setup" ? "Activate facilitator access" : `Open ${portalName}`}</button></form>{!isAdmin && !isFacilitator && <div className="student-register-prompt"><div><b>New to UCC Growth+?</b><span>Create a verified learner profile in the dedicated registration portal.</span></div><a href="/student-registration">Register as a learner</a></div>}{isFacilitator && !inviteToken && <p className="signin-help">A system administrator must issue your one-time facilitator setup invitation before first sign-in.</p>}{isFacilitator && inviteToken && <p className="signin-help invited">Invitation detected. Use the exact institutional email named by the administrator.</p>}</section></section></main>;
}
