...
export async function POST(request: Request) {
  const payload = await request.json() as { mode?: "login" | "register" | "admin_setup" | "facilitator_setup"; portal?: PortalRole; email?: string; fullName?: string; password?: string; returnTo?: string; termsAccepted?: boolean; inviteToken?: string };
  ...
  if (!portal || !["learner", "facilitator", "admin"].includes(portal)) return Response.json({ error: "Choose the Student, Facilitator or Administration sign-in portal." }, { status: 400 });
  ...
  if (payload.mode === "register") {
    if (portal !== "learner") return Response.json({ error: "Student account registration is available only through the Student Registration Portal." }, { status: 403 });
    ...
    if (initialAdminEmail && email === initialAdminEmail) return Response.json({ error: "The configured administrator email cannot be used for student registration." }, { status: 403 });
    ...
    if (!payload.inviteToken || !user.invite_token_hash || await hashToken(payload.inviteToken) !== user.invite_token_hash) return Response.json({ error: "The facilitator invitation is invalid. Open the original link sent by the administrator." }, { status: 403 });
  ...
  if (!allowed) {
    const actualPortal = user?.role === "learner" ? "Student" : user?.role === "facilitator" ? "Facilitator" : user?.role === "admin" ? "Administration" : "assigned";
    const requestedPortal = portal === "learner" ? "Student" : portal === "facilitator" ? "Facilitator" : "Administration";
    return Response.json({ error: `This account belongs to the ${actualPortal} Portal and cannot sign in through the ${requestedPortal} Portal.` }, { status: 403 });
  }
