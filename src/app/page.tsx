...
import { UccBrandLockup, UccGrowthWordmark } from "@/components/ucc-brand";
...
import {
  certificateAchievementStatement, certificateAwardOptions, certificateConfigurationFromSnapshot, certificateEyebrow,
  certificateIssuanceOptions, certificatePartnershipStatement, certificateTitle, isCpdAward, requiresPartner,
  requiresPartnerSignatory, safeCertificateFilename, validateCertificateConfiguration,
  ...
  return (
    <main className="portal-shell">
      <header className="topbar">
        <div className="brand"><UccBrandLockup compact subtitle={`${roleLabel} Portal · Learn · Demonstrate · Progress`} /></div>
...
function DedicatedFacilitatorStudio({ profile, query, setQuery, initials }: { profile: AccountProfile; query: string; setQuery: (value: string) => void; initials: string }) {
  return <main className="dedicated-studio-portal">
    <header className="studio-portal-header">
      <a className="studio-portal-brand" href="/"><UccBrandLockup compact inverse subtitle="Facilitator Course Studio" /></a>
...
const certificateDate = (value: string) => new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "long", year: "numeric" }).format(new Date(value));
...
async function embedCertificateImages(root: HTMLElement) {
  const originals = await Promise.all(Array.from(root.querySelectorAll("img")).map(async (image) => {
    ...
    if (source && !source.startsWith("data:")) {
      ...
      if (!response.ok) throw new Error(`Image could not be prepared (${response.status}).`);
      image.removeAttribute("srcset"); image.src = await blobAsDataUrl(await response.blob());
    }
...
function CertificateCard({ certificate }: { certificate: CertificateRecord }) {
  ...
  const signatureUrl = (key: string) => `/api/certificate-signatures?key=${encodeURIComponent(key)}`;
  const brandingUrl = (key: string) => `/api/certificate-branding?key=${encodeURIComponent(key)}`;
  const printCertificate = async () => {
    const target = expandedFrameRef.current ?? previewFrameRef.current;
    ...
    const cleanup = () => {
      ...
      document.body.classList.remove("certificate-printing"); target.classList.remove("certificate-print-target");
      document.title = oldTitle; restoreImages(); setPrinting(false);
    };
    try {
      restoreImages = await embedCertificateImages(target);
      document.querySelectorAll(".certificate-print-target").forEach((element) => element.classList.remove("certificate-print-target"));
      target.classList.add("certificate-print-target"); document.body.classList.add("certificate-printing");
      document.title = safeCertificateFilename(certificate.course_code).replace(/\.pdf$/i, "");
      window.addEventListener("afterprint", cleanup, { once: true });
      await new Promise<void>((resolve) => requestAnimationFrame(() => requestAnimationFrame(() => resolve())));
      window.print(); window.setTimeout(cleanup, 120_000);
    } catch (error) {
      cleanup(); toast.error(error instanceof Error ? error.message : "The certificate could not be prepared for printing.");
    }
  };
  const partnerIsIssuer = configuration.issuanceModel === "partner_issued";
  const showPartnerBrand = Boolean(configuration.partnerLogoKey && configuration.issuanceModel !== "ucc_sponsored");
  const showUccSignatory = !partnerIsIssuer;
  const showAcademicLead = Boolean(certificate.show_academic_lead && !["jointly_issued", "partner_issued"].includes(configuration.issuanceModel));
  const certificateFrame = (ref: React.RefObject<HTMLDivElement | null>) => <div className="certificate-template-frame" ref={ref} data-print-filename={safeCertificateFilename(certificate.course_code)}>
      <header className="certificate-template-header">
        {!partnerIsIssuer && <div className="certificate-institution-lockup"><Image unoptimized width={74} height={74} src="/ucc_crest_approved_2026.png" className="certificate-crest" alt="University of Cape Coast crest" /><div><b>UNIVERSITY OF CAPE COAST</b><span className="certificate-growth-label">UCC GROWTH<sup>+</sup> · LIFELONG LEARNING</span></div></div>}
        {partnerIsIssuer && <div className="certificate-institution-lockup partner-primary">{configuration.partnerLogoKey && <Image unoptimized width={160} height={74} src={brandingUrl(configuration.partnerLogoKey)} className="certificate-partner-logo" alt={`${configuration.partnerName} logo`} />}<div><b>{configuration.partnerName}</b><span>ISSUING INSTITUTION</span></div></div>}
        {showPartnerBrand && !partnerIsIssuer && <div className="certificate-partner-mark"><span>{configuration.issuanceModel === "jointly_issued" ? "JOINTLY ISSUED WITH" : "IN PARTNERSHIP WITH"}</span><Image unoptimized width={150} height={64} src={brandingUrl(configuration.partnerLogoKey)} className="certificate-partner-logo" alt={`${configuration.partnerName} logo`} /></div>}
        {partnerIsIssuer && <div className="certificate-platform-mark"><Image unoptimized width={42} height={42} src="/ucc_crest_approved_2026.png" alt="University of Cape Coast crest" /><span>Delivered through<br /><b className="ucc-growth-wordmark compact">UCC <span>Growth<sup>+</sup></span></b></span></div>}
      </header>
      <div className="certificate-template-content"><p className="eyebrow">{certificateEyebrow(configuration.awardType, certificate.credential_type === "stacked_credential")}</p><h2>{certificateTitle(configuration.awardType)}</h2><i className="certificate-gold-rule" /><p>This certificate is presented to</p><h3>{certificate.learner_name}</h3><p>{certificateAchievementStatement(configuration.awardType)}</p><h4>{certificate.course_title}</h4><small>{certificatePartnershipStatement(configuration)}</small>{configuration.cpdHours > 0 && <div className="certificate-cpd"><b>{configuration.cpdHours} CPD hour{configuration.cpdHours === 1 ? "" : "s"}</b>{configuration.cpdPoints > 0 && <span>{configuration.cpdPoints} approved CPD point{configuration.cpdPoints === 1 ? "" : "s"}</span>}</div>}</div>
      <footer className="certificate-template-footer">
        {showUccSignatory && <div className={`certificate-signature-block ${certificate.provost_signature_key ? "signed" : "signature-line-only"}`}>{certificate.provost_signature_key && <Image unoptimized width={150} height={45} src={signatureUrl(certificate.provost_signature_key)} alt="Authorised UCC signature" />}<b>{certificate.provost_name || "Authorised University Signatory"}</b><i /><span>{certificate.provost_title || "University of Cape Coast"}</span></div>}
        {configuration.partnerSignatoryName && <div className="certificate-signature-block">{configuration.partnerSignatureKey && <Image unoptimized width={150} height={45} src={brandingUrl(configuration.partnerSignatureKey)} alt="Authorised partner signature" />}<b>{configuration.partnerSignatoryName}</b><i /><span>{configuration.partnerSignatoryTitle}</span></div>}
        {showAcademicLead && <div className={`certificate-signature-block ${certificate.facilitator_signature_key ? "signed" : "signature-line-only"}`}>{certificate.facilitator_signature_key && <Image unoptimized width={150} height={45} src={signatureUrl(certificate.facilitator_signature_key)} alt="Academic lead signature" />}<b>{certificate.facilitator_name || "Course Academic Lead"}</b><i /><span>{certificate.facilitator_title || "Academic lead"}</span></div>}
        <div className="certificate-verification-meta"><i /><span><b>Issued:</b> {certificateDate(certificate.issued_at)}</span><span><b>Credential ID:</b> {certificate.certificate_code}</span><a href={verificationPath} target="_blank" rel="noreferrer">Verify this credential online</a></div><div className="certificate-qr-block"><VerificationQr code={certificate.certificate_code} /><span>SCAN TO VERIFY</span></div>
      </footer>
    </div>;
  const requirements = certificate.requirements?.length ? <div className="certificate-requirements">{certificate.requirements.map((requirement) => <span key={requirement.id ?? requirement.label}><CheckCircle2 /> {requirement.label}</span>)}</div> : null;
  return <article className={`certificate-card ${active ? "" : "revoked"}`}>
    <div className="certificate-preview-button">
      <div className="certificate-preview-canvas">{certificateFrame(previewFrameRef)}</div>
      <button className="certificate-preview-open" type="button" onClick={() => setExpanded(true)} aria-label={`Open ${certificate.course_title} certificate`}><span className="certificate-preview-affordance"><Eye /> View full certificate</span></button>
    </div>
    ...
    <Dialog open={expanded} onOpenChange={setExpanded}>
      <DialogContent className="certificate-detail-dialog">
        ...
        <div className="certificate-expanded-scroll">{certificateFrame(expandedFrameRef)}</div>
...
function PortalLoading() {
  return <main className="access-shell"><section className="access-card loading-card"><UccBrandLockup subtitle="Professional and lifelong learning" /><p className="eyebrow">UCC MICROCREDENTIALS</p><h1>Opening your learning portal</h1><p>Checking your account and access permissions…</p><div className="loading-bar"><span /></div></section></main>;
}
...
function PublicLanding({ inviteToken }: { inviteToken: string }) {
  ...
  const [catalogue, setCatalogue] = useState<PublicCourse[]>([]); const [query, setQuery] = useState(""); const [activeDiscipline, setActiveDiscipline] = useState("All");
  ...
  const publicDisciplines = ["All", ...Array.from(new Set(catalogue.map((course) => course.discipline))).slice(0, 7)];
  const filtered = catalogue.filter((course) => (activeDiscipline === "All" || course.discipline === activeDiscipline) && `${course.title} ${course.code} ${course.discipline} ${course.description}`.toLowerCase().includes(query.toLowerCase()));
  const featured = catalogue[0];
  return <main className="public-micro-site">
    <div className="public-announcement"><span>Open professional learning for everyone - no UCC student status required</span><a href="/verify-credential">Verify a credential <ChevronRight /></a></div>
    <header className="public-header"><a className="public-brand" href="/"><Image unoptimized width={54} height={54} src="/ucc_crest_approved_2026.png" alt="University of Cape Coast crest" /><div><b className="ucc-growth-wordmark"><UccGrowthWordmark /></b><small>Professional and lifelong learning</small></div></a><nav><a href="#subjects">Subjects</a><a href="#courses">Courses</a><a href="/verify-credential">Credentials</a><a href="/facilitator-signin">For educators</a></nav><div className="public-auth"><a href="/student-signin">Log in</a><a className="public-register" href="/student-registration">Join for free</a></div></header>
    <section className="public-hero">
      <div className="public-hero-copy"><p className="eyebrow">UNIVERSITY OF CAPE COAST · LEARNING FOR EVERY STAGE</p><h1>Grow your skills.<br /><span>Prove what you can do.</span></h1><p>Learn through focused online and blended courses created by UCC and trusted partners. Build practical capability and earn credentials employers can verify.</p><label className="public-hero-search"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="What do you want to learn?" /><a href="#courses">Search courses</a></label><div className="public-hero-actions"><a href="#courses">Explore courses <ChevronRight /></a><a href="/student-registration">Create a learner account</a></div><small><CheckCircle2 /> Open enrolment &nbsp; <CheckCircle2 /> Flexible study &nbsp; <CheckCircle2 /> Verifiable achievement</small></div>
      <aside className="public-hero-stage"><div className="hero-orbit one" /><div className="hero-orbit two" /><article><span>FEATURED LEARNING</span><div className="hero-course-icon"><Sparkles /></div><small>{featured?.discipline || "Professional development"}</small><b>{featured?.title || "Career-relevant microcredentials for a changing world"}</b><p>{featured ? `${featured.design?.expectedHours ?? 0} learning hours · ${featured.design?.deliveryPattern ?? "Flexible"}` : "Short, quality-assured learning with evidence you can share."}</p><a href={featured ? "#courses" : "/student-registration"}>{featured ? "View featured course" : "Start learning"} <ChevronRight /></a></article><div className="hero-credential-card"><ShieldCheck /><div><b>Verified credentials</b><span>Shareable, scannable and backed by a live register</span></div></div></aside>
    </section>
    <section className="public-trust-strip"><p>Learning designed for progress</p><div><span><b>University-led</b>Quality-assured programmes</span><span><b>Partner-ready</b>Joint, CPD and sponsored delivery</span><span><b>Evidence-backed</b>Assessment before achievement</span><span><b>Portable</b>Public credential verification</span></div></section>
    <section id="subjects" className="public-subjects"><div><p className="eyebrow">EXPLORE BY SUBJECT</p><h2>Find learning that moves you forward</h2><p>Browse across professional fields, emerging technologies and community priorities.</p></div><div className="public-subject-chips">{publicDisciplines.map((discipline, index) => <button key={discipline} className={activeDiscipline === discipline ? "active" : ""} onClick={() => { setActiveDiscipline(discipline); document.getElementById("courses")?.scrollIntoView({ behavior: "smooth" }); }}>{index % 4 === 0 ? <BookOpen /> : index % 4 === 1 ? <Activity /> : index % 4 === 2 ? <Code2 /> : <Users />}<span>{discipline}</span><ChevronRight /></button>)}</div></section>
    <section className="public-promo"><article><Award /><div><b>Recognised achievement</b><span>Earn an award suited to the learning: microcredential, CPD, participation, attendance or approved professional certification.</span></div></article><article><FlaskConical /><div><b>Applied learning</b><span>Practise through authentic tasks, projects, virtual practicals and workplace evidence.</span></div></article><article><Clock3 /><div><b>Built for working lives</b><span>Choose asynchronous, live or blended experiences with clear effort and outcome information.</span></div></article></section>
    <section id="courses" className="public-catalogue"><div className="public-section-title"><div><p className="eyebrow">POPULAR AND NEW</p><h2>Courses from UCC and trusted partners</h2><p>Compare the skills, learning time and award before you enrol.</p></div><label><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search courses, subjects or skills" /></label></div><div className="public-course-grid">{filtered.map((course, index) => <article key={course.id}><div className={`public-course-visual tone-${index % 4}`}><span>{course.design?.certificate?.awardType?.replaceAll("_", " ") || "microcredential"}</span><Award /></div><div className="public-course-card-body"><header><span>{course.code}</span><em>{course.discipline}</em></header><h3>{course.title}</h3><p>{course.description}</p><div className="public-course-meta"><span><Clock3 /> {course.design?.expectedHours ?? 0} hours</span><span><Gauge /> {course.design?.level ?? "Applied"}</span><span><BookOpen /> {course.design?.deliveryPattern ?? "Flexible"}</span></div><details><summary>View outcomes and course details <ChevronDown /></summary><div><b>What you will be able to do</b><ul>{(course.design?.outcomes ?? []).slice(0,4).map((item) => <li key={item.id}>{item.statement}</li>)}</ul></div></details><footer><small><b>University of Cape Coast</b>Facilitated by {course.facilitatorName}</small><a href="/student-registration">View course <ChevronRight /></a></footer></div></article>)}{filtered.length === 0 && <div className="empty-state wide">No approved public courses match this search yet.</div>}</div></section>
    <section className="public-pathway"><div><p className="eyebrow">START SMALL. KEEP GROWING.</p><h2>Build a learning pathway around your goals.</h2><p>Complete a focused course now, add further credentials over time, and keep your achievements together in a private skills passport.</p></div><a href="/student-registration">Start your learning journey <ChevronRight /></a></section>
    <footer className="public-footer"><div className="public-brand"><Image unoptimized width={46} height={46} src="/ucc_crest_approved_2026.png" alt="University of Cape Coast crest" /><div><b className="ucc-growth-wordmark"><UccGrowthWordmark /></b><small>University of Cape Coast</small></div></div><span>Quality-assured professional and lifelong learning</span><nav><a href="#courses">Explore</a><a href="/verify-credential">Verify</a><a href="/privacy">Privacy</a><a href="/accessibility">Accessibility</a><a href="/support">Support</a><a href="/student-signin">Log in</a></nav></footer>
  </main>;
}
...
function PortalAccess({ inviteToken }: { inviteToken: string }) {
  ...
  return <main className="access-shell commercial-access"><section className="access-card access-selector"><div className="access-brand"><UccBrandLockup subtitle="University of Cape Coast" /></div><div className="access-product-heading"><div><p className="eyebrow">SECURE ROLE PORTALS</p><h1>{inviteToken ? "Complete your facilitator invitation." : "One platform. Three focused workspaces."}</h1><p>{inviteToken ? "Sign in with the exact professional email invited by the system administrator, then complete permanent biodata and identity verification." : "Learners develop and demonstrate skills. Facilitators design and assess. Administrators govern quality and trust."}</p></div>{!inviteToken && <a href="/verify-credential"><QrCode /> Verify a credential</a>}</div>{inviteToken ? <a className="portal-access-card facilitator invited" href={signInUrl("facilitator")} target="_top"><span className="portal-access-icon"><ShieldCheck /></span><span><b>Complete facilitator setup</b><small>Use the professional email address named in your invitation.</small></span><ChevronRight /></a> : <div className="commercial-role-grid"><article className="commercial-role-card student"><header><span><BookOpen /></span><em>FOR LEARNERS</em></header><h2>Learner Portal</h2><p>Discover courses, complete authentic assessments, build a private skills passport and share verified credentials.</p><ul><li><CheckCircle2 /> Stackable microcredentials</li><li><CheckCircle2 /> Virtual labs and Colab</li><li><CheckCircle2 /> Portable credential wallet</li></ul><div><a className="role-primary" href="/student-registration">Register as a learner <ChevronRight /></a><a className="role-secondary" href={signInUrl("learner")}>Learner sign in</a></div></article><article className="commercial-role-card facilitator"><header><span><ShieldCheck /></span><em>FOR EDUCATORS</em></header><h2>Facilitator Portal</h2><p>Author quality-assured programmes, facilitate cohorts and assess authentic evidence in one workspace.</p><ul><li><CheckCircle2 /> Course and activity studio</li><li><CheckCircle2 /> Marking and competency decisions</li><li><CheckCircle2 /> Cohort intelligence</li></ul><div><a className="role-primary" href={signInUrl("facilitator")}>Facilitator sign in <ChevronRight /></a><small>Administrator invitation required</small></div></article><article className="commercial-role-card admin"><header><span><Users /></span><em>FOR GOVERNANCE</em></header><h2>System Administration</h2><p>Control identities, academic approvals, credential status and institutional performance.</p><ul><li><CheckCircle2 /> User and access governance</li><li><CheckCircle2 /> Credential registry</li><li><CheckCircle2 /> Platform analytics</li></ul><div><a className="role-primary" href={signInUrl("admin")}>Administrator sign in <ChevronRight /></a><small>Restricted system role</small></div></article></div>}<footer className="access-trust"><ShieldCheck /><span>Role permissions are enforced after sign-in. Portal selection never grants access.</span></footer></section></main>;
}
...
function StudentRegistrationHandoff({ email }: { email: string }) {
  return <main className="access-shell"><section className="access-card registration-handoff"><div className="pending-ring student"><BookOpen /></div><p className="eyebrow">LEARNER REGISTRATION REQUIRED</p><h1>Finish setting up your Learner Portal.</h1><p>Your secure sign-in is ready for <b>{email}</b>. Continue in the dedicated registration portal to complete your profile, learning interests, accessibility preferences and identity verification.</p><div className="registration-handoff-features"><span><CheckCircle2 /> Personal learner number</span><span><CheckCircle2 /> Verified learning identity</span><span><CheckCircle2 /> Skills passport and credential wallet</span></div><a className="access-primary" href="/student-registration">Continue learner registration <ChevronRight /></a><a className="access-secondary" href="/signout-with-chatgpt?return_to=%2F">Sign out and use another account</a></section></main>;
}
...
type FacilitatorRecord = { email: string; full_name: string; organisation?: string | null; status: string; identity_status: string; created_at: string };
...
function AdminPortal({ onOpenRegister }: { onOpenRegister: () => void }) {
  ...
  const createFacilitator = async () => {
    if (!fullName.trim() || !email.trim() || !institution.trim()) return toast.error("Enter the facilitator’s full name, professional email and institution.");
    setSaving(true);
    try {
      const response = await fetch("/api/admin/facilitators", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ fullName, email, institution }) });
      const result = await response.json() as { error?: string; inviteUrl?: string; expiresAt?: string; emailSent?: boolean };
      ...
      setFullName(""); setEmail(""); setInstitution(""); await load();
      toast.success(result.emailSent ? "Facilitator invitation emailed" : "Facilitator setup link created", { description: result.emailSent ? "The one-time link was sent to the approved professional email." : "Email delivery was unavailable. Copy and send the one-time link securely." });
  ...
  return <div className="admin-portal-stack"><PlatformModeControl /><div className="admin-layout">
    <div className="admin-main-stack">
      <section className="page-panel">
        ...
        <div className="admin-stats"><article><Users /><div><b>{counts.learner ?? 0}</b><span>Learners</span></div></article><article><ShieldCheck /><div><b>{counts.facilitator ?? 0}</b><span>Facilitators</span></div></article><article><FileCheck2 /><div><b>{verifications.length}</b><span>Pending ID reviews</span></div></article></div>
        <div className="admin-table"><div className="admin-table-head"><span>Facilitator</span><span>Status</span><span>Created</span></div>{loading && <div className="empty-state">Loading facilitator accounts…</div>}{!loading && facilitators.map((facilitator) => <article key={facilitator.email}><div><b>{facilitator.full_name}</b><span>{facilitator.email}{facilitator.organisation ? ` · ${facilitator.organisation}` : ""}</span></div><em>{facilitator.status.replaceAll("_", " ")}</em><time>{new Date(facilitator.created_at).toLocaleDateString()}</time></article>)}{!loading && facilitators.length === 0 && <div className="empty-state">No facilitator accounts have been created yet.</div>}</div>
      </section>
...
function FacilitatorStudio({ email, query, setQuery }: { email: string; query: string, setQuery: (value: string) => void }) {
  ...
  const [certificateAssetUploading, setCertificateAssetUploading] = useState<"partner_logo" | "partner_signature" | null>(null);
  ...
  const uploadCertificateAsset = async (file: File, assetType: "partner_logo" | "partner_signature") => {
    setCertificateAssetUploading(assetType);
    try {
      const body = new FormData(); body.append("assetType", assetType); body.append("file", file);
      const response = await fetch("/api/certificate-branding", { method: "POST", body });
      const result = await response.json() as { key?: string; error?: string };
      if (!response.ok || !result.key) throw new Error(result.error ?? "The certificate image could not be saved.");
      setDesign((current) => ({ ...current, certificate: { ...current.certificate, [assetType === "partner_logo" ? "partnerLogoKey" : "partnerSignatureKey"]: result.key! } }));
      toast.success(assetType === "partner_logo" ? "Partner logo saved" : "Partner signature saved");
    } catch (error) { toast.error(error instanceof Error ? error.message : "The certificate image could not be saved."); }
    finally { setCertificateAssetUploading(null); }
  };
  ...
  return <div className="authoring-shell" id="course-studio-editor">
    <section className="authoring-main page-panel">
      ...
      {step === "details" && <div className="authoring-form commercial-blueprint-form">
        ...
        <section className="programme-governance-metadata certificate-issuance-editor">
          <div className="studio-section-heading compact"><div><p className="eyebrow">CERTIFICATE ISSUANCE</p><h3>Choose the award and institutional relationship</h3><p>The certificate shows only the essential award facts. Course code, delivery mode, credit, criteria and governance evidence remain available in the verification portal.</p></div><Award /></div>
          <div className="governance-record-grid">
            <label>Award type<select value={design.certificate.awardType} onChange={(event) => { const awardType = event.target.value as CourseDesign["certificate"]["awardType"]; setDesign((current) => ({ ...current, certificate: { ...current.certificate, awardType, cpdHours: isCpdAward(awardType) && current.certificate.cpdHours <= 0 ? current.expectedHours : current.certificate.cpdHours } })); }}>{certificateAwardOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><span>{certificateAwardOptions.find((option) => option.value === design.certificate.awardType)?.description}</span></label>
            <label>Issuance relationship<select value={design.certificate.issuanceModel} onChange={(event) => setDesign((current) => ({ ...current, certificate: { ...current.certificate, issuanceModel: event.target.value as CourseDesign["certificate"]["issuanceModel"] } }))}>{certificateIssuanceOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><span>{certificateIssuanceOptions.find((option) => option.value === design.certificate.issuanceModel)?.description}</span></label>
            {requiresPartner(design.certificate.issuanceModel) && <><label>Partner / sponsor institution<input value={design.certificate.partnerName} onChange={(event) => setDesign((current) => ({ ...current, certificate: { ...current.certificate, partnerName: event.target.value } }))} placeholder="Full legal or approved public name" /></label><label className="certificate-asset-upload">Partner logo <span>{design.certificate.partnerLogoKey ? "Logo uploaded" : requiresPartnerSignatory(design.certificate.issuanceModel) ? "Required for this issuance model" : "Optional"}</span><input type="file" accept="image/png,image/jpeg,image/webp" disabled={Boolean(certificateAssetUploading)} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadCertificateAsset(file, "partner_logo"); event.currentTarget.value = ""; }} /></label></>}
            {isCpdAward(design.certificate.awardType) && <><label>Verified CPD learning hours<input type="number" min="0.5" max="2000" step="0.5" value={design.certificate.cpdHours} onChange={(event) => setDesign((current) => ({ ...current, certificate: { ...current.certificate, cpdHours: Math.max(0, Number(event.target.value)) } }))} /></label><label>Approved CPD points (optional)<input type="number" min="0" max="2000" step="0.5" value={design.certificate.cpdPoints} onChange={(event) => setDesign((current) => ({ ...current, certificate: { ...current.certificate, cpdPoints: Math.max(0, Number(event.target.value)) } }))} /><span>Only enter points formally approved by the named body.</span></label></>}
            {requiresPartnerSignatory(design.certificate.issuanceModel) && <><label>Partner authorised signatory<input value={design.certificate.partnerSignatoryName} onChange={(event) => setDesign((current) => ({ ...current, certificate: { ...current.certificate, partnerSignatoryName: event.target.value } }))} /></label><label>Partner signatory title<input value={design.certificate.partnerSignatoryTitle} onChange={(event) => setDesign((current) => ({ ...current, certificate: { ...current.certificate, partnerSignatoryTitle: event.target.value } }))} /></label><label className="certificate-asset-upload wide-record-field">Authorised partner signature <span>{design.certificate.partnerSignatureKey ? "Signature uploaded" : "PNG, JPEG or WebP; maximum 2 MB"}</span><input type="file" accept="image/png,image/jpeg,image/webp" disabled={Boolean(certificateAssetUploading)} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadCertificateAsset(file, "partner_signature"); event.currentTarget.value = ""; }} /></label></>}
            {(design.certificate.awardType === "professional_certification" || design.certificate.cpdPoints > 0) && <><label>Approving professional / regulatory body<input value={design.certificate.approvalBody} onChange={(event) => setDesign((current) => ({ ...current, certificate: { ...current.certificate, approvalBody: event.target.value } }))} /></label><label>Approval / accreditation reference<input value={design.certificate.approvalReference} onChange={(event) => setDesign((current) => ({ ...current, certificate: { ...current.certificate, approvalReference: event.target.value } }))} /></label></>}
            {!requiresPartnerSignatory(design.certificate.issuanceModel) && <label className="certificate-academic-lead wide-record-field"><Switch checked={design.certificate.showAcademicLead} onCheckedChange={(showAcademicLead) => setDesign((current) => ({ ...current, certificate: { ...current.certificate, showAcademicLead } }))} /><span><b>Add the course academic lead as a second signatory</b><small>The facilitator’s approved signature will be snapshotted when the credential is issued.</small></span></label>}
          </div>
          <div className={`certificate-model-check ${certificateValidation.valid ? "valid" : "invalid"}`}>{certificateValidation.valid ? <CheckCircle2 /> : <AlertTriangle />}<div><b>{certificateValidation.valid ? "Certificate model complete" : "Complete the certificate model before submission"}</b>{certificateValidation.issues.map((issue) => <span key={issue}>{issue}</span>)}</div></div>
        </section>
...
function SupportForm({ role, onClose }: { role: PortalRole; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.reference) throw new Error(result.error ?? "The support request could not be submitted.");
      toast.success("Support request submitted", { description: `Reference ${result.reference}` });
      onClose();
    } catch (error) { toast.error(error instanceof Error ? error.message : "The support request could not be submitted."); }
    finally { setSaving(false); }
  };
  return <div className="support-form"><label>How can we help?<select value={category} onChange={(event) => setCategory(event.target.value)}>{role === "learner" ? <><option value="learning">Learning activity</option><option value="assessment">Assessment or feedback</option><option value="technical">Technical access</option></> : role === "facilitator" ? <><option value="course">Course design or publishing</option><option value="assessment">Assessment workflow</option><option value="technical">Technical access</option></> : <><option value="access">User or access governance</option><option value="quality">Academic quality workflow</option><option value="technical">Platform operations</option></>}</select></label><label>Message<textarea value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Describe the issue and the support team will respond." /></label><button className="dialog-primary" disabled={saving || message.trim().length < 10} onClick={submit}>{saving ? "Submitting…" : "Send support request"}</button></div>;
}
...
function UtilityDialog({ value, profile, onProfileUpdated, onClose }: { value: "notifications" | "support" | "preferences" | "profile" | "assessment" | null; profile: AccountProfile; onProfileUpdated: (profile: AccountProfile) => void; onClose: () => void }) {
  ...
  return <Dialog open={Boolean(value)} onOpenChange={(open) => !open && onClose()}><DialogContent className={value === "profile" ? "utility-dialog profile-dialog" : "utility-dialog"}><DialogHeader><p className="eyebrow">UCC MICROCREDENTIALS</p><DialogTitle>{title}</DialogTitle><DialogDescription>{value === "assessment" ? "Complete the objective item and optional essay response." : value === "profile" ? "Complete biodata and submit current identity evidence for restricted UCC verification." : "Manage this area without leaving your role-protected workspace."}</DialogDescription></DialogHeader>
    ...
    {value === "support" && <SupportForm role={profile.role} onClose={onClose} />}
