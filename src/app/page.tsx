  ...
  return (
    <main className="portal-shell">
      <header className="topbar">
        <div className="brand"><div className="brand-mark"><GraduationCap size={25} /></div><div><strong className="ucc-growth-wordmark"><UccGrowthWordmark /></strong><span>{roleLabel} Portal · Learn · Demonstrate · Progress</span></div></div>
...
function DedicatedFacilitatorStudio({ profile, query, setQuery, initials }: { profile: AccountProfile; query: string; setQuery: (value: string) => void; initials: string }) {
  return <main className="dedicated-studio-portal">
    <header className="studio-portal-header">
      <a className="studio-portal-brand" href="/"><span><GraduationCap /></span><div><b className="ucc-growth-wordmark"><UccGrowthWordmark /></b><small>Facilitator Course Studio</small></div></a>
...
function CertificateCard({ certificate }: { certificate: CertificateRecord }) {
  ...
  const printCertificate = async () => {
    if (!frameRef.current || printing) return;
    setPrinting(true);
    ...
    const cleanup = () => {
      ...
      document.body.classList.remove("certificate-printing"); frameRef.current?.classList.remove("certificate-print-target");
      document.title = oldTitle; restoreImages(); setPrinting(false);
    };
    try {
      restoreImages = await embedCertificateImages(frameRef.current);
      document.querySelectorAll(".certificate-print-target").forEach((element) => element.classList.remove("certificate-print-target"));
      frameRef.current.classList.add("certificate-print-target"); document.body.classList.add("certificate-printing");
  ...
  const showAcademicLead = Boolean(certificate.show_academic_lead && certificate.facilitator_name && !["jointly_issued", "partner_issued"].includes(configuration.issuanceModel));
  return <article className={`certificate-card ${active ? "" : "revoked"}`}>
    <div className="certificate-template-frame" ref={frameRef} data-print-filename={safeCertificateFilename(certificate.course_code)}>
      ...
      <footer className="certificate-template-footer">
        {certificate.provost_name && <div className="certificate-signature-block">{certificate.provost_signature_key && <Image unoptimized width={150} height={45} src={signatureUrl(certificate.provost_signature_key)} alt="Authorised UCC signature" />}<b>{certificate.provost_name}</b><i /><span>{certificate.provost_title || "Authorised University signatory"}</span></div>}
        ...
        {showAcademicLead && <div className="certificate-signature-block">{certificate.facilitator_signature_key && <Image unoptimized width={150} height={45} src={signatureUrl(certificate.facilitator_signature_key)} alt="Academic lead signature" />}<b>{certificate.facilitator_name}</b><i /><span>{certificate.facilitator_title || "Academic lead"}</span></div>}
