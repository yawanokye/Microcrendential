...
function FacilitatorStudio({ email, query, setQuery }: { email: string; query: string, setQuery: (value: string) => void }) {
  ...
  const loadDraft = async (course: StudioDraft) => {
    ...
    if (course.status === "active") {
      ...
      if (result.revision.status === "pending_review") return toast.info("This revision is awaiting a committee decision record", { description: "The published course remains active. Editing will reopen after the administrator returns or applies the revision." });
      ...
      toast.success(`Revision ${result.revision.revisionNumber} opened`, { description: "The live course stays unchanged until the approved amendment decision is recorded and applied." });
  ...
  const resetQuestionEditor = () => {
    ...
    setQuestionPoints(1); setMarkingScheme(""); setWhiteboardEnabled(false); setFeedbackCorrect("Well done. You have demonstrated the required understanding.");
  ...
  const loadQuestionIntoEditor = (question: AssessmentQuestion) => {
    ...
    setQuestionPoints(question.points || 1); setQuestionOutcomeIds(question.outcomeIds?.length ? question.outcomeIds : (design.outcomes[0]?.id ? [design.outcomes[0].id] : []));
    setMarkingScheme(question.scheme ?? ""); setFeedbackCorrect(question.feedbackCorrect ?? ""); setFeedbackIncorrect(question.feedbackIncorrect ?? ""); setLearnerAdvice(question.learnerAdvice ?? ""); setWhiteboardEnabled(Boolean(question.whiteboardEnabled));
  ...
  const openMaterialEditor = (material: Material) => {
    const index = materials.indexOf(material); if (index < 0) return; setMaterialEdit({ index, material: { ...material, outcomeIds: [...(material.outcomeIds ?? [])] } });
  };
  const saveMaterialEditor = () => {
    if (!materialEdit) return; const title = materialEdit.material.title.trim(); if (!title) return toast.error("Give the learning block a title.");
    setMaterials((items) => items.map((item, index) => index === materialEdit.index ? { ...materialEdit.material, title } : item));
    setMaterialEdit(null); toast.success("Learning content updated", { description: "Preview the block again before submitting the course." });
  };
  ...
  return <div className="authoring-shell">
    <section className="authoring-main page-panel">
      ...
      {step === "assessment" && <div className="assessment-builder">
        ...
        <section className="question-bank-summary"><p className="eyebrow">QUESTION BANK REVIEW</p><h3>{questions.length} authored question{questions.length === 1 ? "" : "s"}</h3><p>Preview every question, then accept it into the editor to revise the question, answers, marking guidance or feedback before saving.</p>{questions.map((question, index) => <article key={question.id}><span>{index + 1}</span><div><b>{question.type} · {question.points} point{question.points === 1 ? "" : "s"}{question.whiteboardEnabled ? " · Mathematical whiteboard" : ""}</b><p>{question.prompt}</p></div><div className="question-bank-actions"><button type="button" onClick={() => setPreviewQuestion(question)}><Eye /> Preview</button><button type="button" onClick={() => loadQuestionIntoEditor(question)}><Pencil /> Edit</button><button type="button" className="danger" onClick={() => { setQuestions((items) => items.filter((item) => item.id !== question.id)); if (editingQuestionId === question.id) resetQuestionEditor(); }}>Remove</button></div></article>)}{questions.length === 0 && <div className="empty-state">No typed questions yet. Use the editor below or upload a question bank.</div>}</section></div>
