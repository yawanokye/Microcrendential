...
function FacilitatorStudio({ email, query, setQuery }: { email: string; query: string, setQuery: (value: string) => void }) {
  ...
  const loadDraft = async (course: StudioDraft) => {
    ...
    setDesign(editable.design ?? defaultCourseDesign()); setMaterials(editable.materials ?? []); setCourseActivities((editable.activities ?? []).map((activity) => activity.kind === "inline" ? activity : { ...activity, required: false })); setAssessmentModes(editable.assessmentModes ?? []);
