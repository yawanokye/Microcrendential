...
function CourseApprovalPanel() {
  ...
  return <section className="page-panel course-approval-panel"><div className="page-title"><div><p className="eyebrow">ACADEMIC ACTIVATION</p><h2>Facilitator course approvals</h2><p>Review structured outcomes, content alignment, accessibility and assessment before a version enters the learner catalogue.</p></div><span className="access-badge"><BookOpen /> {items.filter((course) => course.status === "pending_review").length} pending</span></div>{loading && <div className="empty-state">Loading facilitator courses…</div>}<div className="course-approval-list commercial-approval-list">{items.map((course) => { const quality = evaluateCourseQuality({ title: course.title, description: course.description, design: course.design ?? defaultCourseDesign(), materials: course.materials ?? [], questionCount: course.assessmentConfig?.questions?.length ?? 0 }); return <article key={course.id}><div><span>{course.code} · {course.discipline} · version {course.versionNumber} · {course.status.replaceAll("_", " ")}</span><h3>{course.title}</h3><p>{course.facilitatorName} · {course.design?.outcomes?.length ?? 0} outcomes · {course.materials?.length ?? 0} learning blocks · {course.activities?.length ?? 0} applied activities</p><div className="admin-quality-meter"><Progress value={quality.score} /><b>{quality.score}% quality readiness</b></div><div className="admin-course-facts"><span>{course.design?.expectedHours ?? 0} hours</span><span>{course.design?.level ?? "level not set"}</span><span>{course.certificateEnabled ? "UCC QR certificate" : "certificate disabled"}</span></div></div><div><button className="reject" disabled={course.status === "active"} onClick={() => review(course.id, "rejected")}>Return</button><button className="approve" disabled={course.status !== "pending_review" || !quality.ready} onClick={() => review(course.id, "active")}><CheckCircle2 /> Activate version</button></div></article>; })}{!loading && items.length === 0 && <div className="empty-state">No facilitator courses have been submitted.</div>}</div></section>;
}
