...
type CourseActivity = { id: string; kind: "colab" | "virtual_lab"; title: string; instructions: string; required: boolean; passMark: number; attemptsAllowed: number; maxMark?: number; dueAt?: string; rubric?: string; notebookKey?: string; notebookFileName?: string; templateUrl?: string; practicalId?: string; discipline?: string; sectionId?: string; sectionTitle?: string; gradingMode?: "facilitator" | "ai_auto" | "ai_luna" | "ai_terra" };
...
function VideoTranscriptDialog({ material, onClose, onComplete, completeLabel = "Complete and continue", sectionNote = "" }: { material: CourseMaterial | null; onClose: () => void; onComplete?: () => void; completeLabel?: string; sectionNote?: string }) {
...
function PublishedCourseExperience({ course, onOpenActivity, onExit, preview = false }: { course: Course; onOpenActivity: (activity: CourseActivity) => void; onExit?: () => void; preview?: boolean }) {
  ...
  type ActivityStatus = { passed:boolean; status:string; mark:number|null; feedback:string; assessedAt:string|null };
  ...
  if (stage === "materials") {
    ...
    const sectionRequiredActivities=sectionActivities.filter((activity)=>activity.required!==false);
    ...
    return <div className="published-course commercial-learner-course">
      ...
      <button className="dialog-primary" disabled={!questions.length || !requiredComplete || assessmentPassedPreviously} title={!requiredComplete ? "Complete every required lesson and learning activity first." : assessmentPassedPreviously ? "You have already passed this assessment." : undefined} onClick={() => setStage("assessment")}><FileCheck2 /> {!requiredComplete ? "Complete required learning to unlock assessment" : assessmentPassedPreviously ? "Assessment passed · review only" : questions.length ? `Begin ${questions.length}-question assessment` : "Assessment awaiting facilitator"}</button><VideoTranscriptDialog material={selectedMaterial} onClose={() => setSelectedMaterial(null)} onComplete={selectedMaterial && completedMaterialIds.includes(materialId(selectedMaterial, orderedMaterials.indexOf(selectedMaterial))) ? () => setSelectedMaterial(null) : completeLesson} completeLabel={selectedMaterial && completedMaterialIds.includes(materialId(selectedMaterial, orderedMaterials.indexOf(selectedMaterial))) ? "Close lesson" : orderedMaterials.indexOf(selectedMaterial!) < orderedMaterials.length - 1 ? "Complete and continue" : "Complete final lesson"} sectionNote={sections.find((section) => section.id === selectedMaterial?.sectionId)?.description ?? ""} />
    </div>;
  }
...
function FacilitatorStudio({ email, query, setQuery }: { email: string; query: string, setQuery: (value: string) => void }) {
  ...
  const loadDraft = async (course: StudioDraft) => {
    ...
    setGateRequired(editable.gateRequired); setQuestionLimit(editable.questionLimit); setCertificateEnabled(editable.certificateEnabled); setPassMark(editable.assessmentConfig?.passMark ?? 70); setAttempts(editable.assessmentConfig?.attempts ?? "3"); setQuestions((editable.assessmentConfig?.questions ?? []).map((question) => ({ ...question, previewed: question.previewed ?? (course.status === "active"), approved: question.approved ?? (course.status === "active") }))); setQuestionFiles(editable.assessmentConfig?.questionFiles ?? []);
  ...
  const resetQuestionEditor = () => {
    ...
    setQuestionPoints(1); setQuestionGradingMode("rule"); setMarkingScheme(""); setWhiteboardEnabled(false); setFeedbackCorrect("Well done. You have demonstrated the required understanding.");
  ...
  const openMaterialEditor = (material: Material) => {
    const index = materials.indexOf(material); if (index < 0) return; setMaterialPictureUrl(""); setMaterialEdit({ index, material: { ...material, outcomeIds: [...(material.outcomeIds ?? [])] } });
  };
  const runMaterialEditorCommand = (command: string, value?: string) => { materialEditorRef.current?.focus(); document.execCommand(command, false, value); const html = materialEditorRef.current?.innerHTML ?? ""; setMaterialEdit((current) => current ? { ...current, material: { ...current.material, readableHtml: html } } : current); };
  const insertMaterialEditorHtml = (html: string) => { materialEditorRef.current?.focus(); document.execCommand("insertHTML", false, html); const next = materialEditorRef.current?.innerHTML ?? ""; setMaterialEdit((current) => current ? { ...current, material: { ...current.material, readableHtml: next } } : current); };
  ...
  const insertMaterialPictureUrl = () => { try { const parsed = new URL(materialPictureUrl); if (parsed.protocol !== "https:") throw new Error(); insertMaterialEditorHtml(`<figure><img src="${parsed.toString().replace(/"/g, "&quot;")}" alt="Learning picture or diagram"><figcaption>Learning picture or diagram</figcaption></figure>`); setMaterialPictureUrl(""); } catch { toast.error("Use a complete HTTPS image address."); } };
  const uploadMaterialPicture = async (image: File) => { if (!image.type.startsWith("image/")) return toast.error("Choose a browser-readable image file."); try { const body = new FormData(); body.append("file", image); const response = await fetch("/api/uploads", { method: "POST", body }); const result = await response.json() as { key?: string; name?: string; error?: string }; if (!response.ok || !result.key) throw new Error(result.error ?? "The picture could not be uploaded."); const source = `/api/course-materials?key=${encodeURIComponent(result.key)}`; const safeAlt = (result.name || image.name || "Learning illustration").replace(/[<>"']/g, ""); insertMaterialEditorHtml(`<figure><img src="${source}" alt="${safeAlt}"><figcaption>${safeAlt}</figcaption></figure>`); setMaterialEdit((current) => current ? { ...current, material: { ...current.material, inlineAssetKeys: [...new Set([...(current.material.inlineAssetKeys ?? []), result.key!])] } } : current); } catch (error) { toast.error(error instanceof Error ? error.message : "The picture could not be uploaded."); } };
  ...
  return <div className="authoring-shell">
    <section className="authoring-main page-panel">
      ...
      {step === "details" && <div className="authoring-form commercial-blueprint-form">
        ...
        <section className="programme-governance-metadata credential-pathway-editor"><div className="studio-section-heading compact"><div><p className="eyebrow">STACKABLE CREDENTIAL PATHWAY · OPTIONAL</p><h3>Link this microcredential to a broader approved credential</h3><p>The relationship is shown in the learner portal only. It is not printed on this individual microcredential certificate.</p></div><Award /></div><div className="governance-record-grid"><label>Broader credential code<input value={design.broaderCredentialCode} onChange={(event) => setDesign((current) => ({ ...current, broaderCredentialCode: event.target.value.toUpperCase() }))} placeholder="e.g. CDPS-001" /></label><label>Broader credential title<input value={design.broaderCredentialTitle} onChange={(event) => setDesign((current) => ({ ...current, broaderCredentialTitle: event.target.value }))} placeholder="e.g. Certificate in Digital Professional Skills" /></label><label className="wide-record-field">Required component microcredential codes<input value={design.broaderCredentialRequiredCodes.join(", ")} onChange={(event) => setDesign((current) => ({ ...current, broaderCredentialRequiredCodes: event.target.value.split(",").map((item) => item.trim().toUpperCase()).filter(Boolean) }))} placeholder="DIG-LIT-001, DATA-LIT-001, AI-FND-001" /><span>Enter at least two course codes. The learner portal calculates progress from earned component credentials.</span></label></div></section>
