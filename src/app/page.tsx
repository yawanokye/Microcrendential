...
function CertificateCard({ certificate }: { certificate: CertificateRecord }) {
  ...
  return <article className={`certificate-card ${active ? "" : "revoked"}`}>
    ...
    <div className="certificate-signatories"><span>{certificate.facilitator_signature_key && <Image unoptimized width={180} height={60} src={`/api/certificate-signatures?key=${encodeURIComponent(certificate.facilitator_signature_key)}`} alt={`${certificate.facilitator_name ?? "Facilitator"} signature`} />}<i /><b>{certificate.facilitator_name ?? "Authorised Course Facilitator"}</b><small>{certificate.facilitator_title ?? "Course Facilitator"}</small></span><span>{certificate.provost_signature_key && <Image unoptimized width={180} height={60} src={`/api/certificate-signatures?key=${encodeURIComponent(certificate.provost_signature_key)}`} alt={`${certificate.provost_name ?? "Provost"} signature`} />}<i /><b>{certificate.provost_name ?? "Authorised Provost"}</b><small>{certificate.provost_title ?? "Provost, College of Distance Education"}</small></span></div>
...
function PublishedCourseExperience({ course, onOpenActivity, preview = false }: { course: Course; onOpenActivity: (activity: CourseActivity) => void; preview?: boolean }) {
  ...
  const questions = course.assessmentConfig?.questions ?? []; const orderedMaterials = course.materials ?? []; const [stage, setStage] = useState<"materials" | "assessment" | "result">("materials"); const [answers, setAnswers] = useState<Record<string, unknown>>({}); const [videoReady, setVideoReady] = useState<Record<string, boolean>>({}); const [score, setScore] = useState<number | null>(null); const [passed, setPassed] = useState(false); const [awaitingMarking, setAwaitingMarking] = useState(false); const [certificate, setCertificate] = useState<CertificateRecord | null>(null); const [completion, setCompletion] = useState<Completion | null>(null); const [submitting, setSubmitting] = useState(false); const [selectedMaterial, setSelectedMaterial] = useState<CourseMaterial | null>(null); const [completedMaterialIds, setCompletedMaterialIds] = useState<string[]>([]); const [progressLoading, setProgressLoading] = useState(!preview);
