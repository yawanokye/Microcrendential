...
type Course = { code: string; title: string; school: string; progress: number; modules: string; accent: string; next: string; discipline?: string; published?: boolean; description?: string; materials?: CourseMaterial[]; activities?: CourseActivity[]; assessmentConfig?: { passMark?: number; attempts?: string; questions?: AssessmentQuestion[] }; design?: CourseDesign; facilitatorName?: string; createdByEmail?: string; certificateEnabled?: boolean; certificateFeeGhs?: number; status?: string; id?: number; versionNumber?: number; updatedAt?: string };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (parameters.get("payment") === "success") {
      toast.success(parameters.get("purpose") === "certificate" ? "Certificate payment confirmed" : "Enrolment payment confirmed");
      window.history.replaceState({}, "", window.location.pathname);
    } else if (parameters.get("payment") === "failed") {
      toast.error("Payment was not completed", { description: "No access or certificate charge was accepted. You can safely try again." });
      window.history.replaceState({}, "", window.location.pathname);
    }
  ...
  const payAndEnrol = async (courseCode: string) => {
    const response = await fetch("/api/payments/initialize", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ courseCode, purpose: "enrollment" }) });
    const result = await response.json() as { authorizationUrl?: string; completed?: boolean; error?: string };
    if (!response.ok) return toast.error(result.error ?? "Payment could not be started.");
    if (result.authorizationUrl) return window.location.assign(result.authorizationUrl);
    if (result.completed) {
      setAccount((current) => current ? ({ ...current, enrollments: [...new Set([...(current.enrollments ?? []), courseCode])] }) : current);
      toast.success("Enrolment confirmed", { description: `${courseCode} is now in your learning portal.` });
    }
...
function FacilitatorStudioLauncher() {
  return <section className="studio-launcher page-panel">
    ...
    <div className="studio-launcher-journey">{[
      ...
      ["4", "Activities", "Add Colab or virtual practice"],
...
function PublishedCourseExperience({ course, onOpenActivity, preview = false }: { course: Course; onOpenActivity: (activity: CourseActivity) => void; preview?: boolean }) {
  type Completion = { complete: boolean; certificateEnabled?: boolean; certificateFeeGhs?: number; certificatePaymentRequired?: boolean; certificatePaymentComplete?: boolean; certificateReady?: boolean; requirements: { id: string; label: string; complete: boolean; evidence?: string }[] };
  const questions = course.assessmentConfig?.questions ?? []; const [stage, setStage] = useState<"materials" | "assessment" | "result">("materials"); const [answers, setAnswers] = useState<Record<string, unknown>>({}); const [videoReady, setVideoReady] = useState<Record<string, boolean>>({}); const [score, setScore] = useState<number | null>(null); const [passed, setPassed] = useState(false); const [certificate, setCertificate] = useState<CertificateRecord | null>(null); const [completion, setCompletion] = useState<Completion | null>(null); const [submitting, setSubmitting] = useState(false); const [certificateBusy, setCertificateBusy] = useState(false); const [selectedMaterial, setSelectedMaterial] = useState<CourseMaterial | null>(null);
  ...
  const requestCertificate = async () => {
    if (preview) return toast.info("Certificate payment is disabled in preview.");
    setCertificateBusy(true);
    try {
      const response = await fetch("/api/payments/initialize", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ courseCode: course.code, purpose: "certificate" }) });
      const result = await response.json() as { authorizationUrl?: string; certificate?: CertificateRecord; error?: string };
      if (!response.ok) throw new Error(result.error ?? "Certificate request could not be started.");
      if (result.authorizationUrl) return window.location.assign(result.authorizationUrl);
      if (result.certificate) { setCertificate(result.certificate); toast.success("Your UCC certificate is ready."); }
    } catch (error) { toast.error(error instanceof Error ? error.message : "Certificate request could not be completed."); }
    finally { setCertificateBusy(false); }
  };
  if (stage === "result") return <div className="published-course-result">{certificate ? <CertificateCard certificate={certificate} /> : <div className="completion-result"><span className={passed ? "passed" : "retry"}>{passed ? <CheckCircle2 /> : <FileCheck2 />}</span><h3>{passed ? "Assessment passed" : `Assessment result: ${score}%`}</h3><p>{passed ? completion?.complete ? "Your academic completion is recorded. You can now generate the verified UCC certificate under the published certificate terms." : "Your result is recorded. Complete every remaining required activity below to unlock certificate eligibility." : "Review the learning materials and try the assessment again when ready."}</p>{completion?.requirements?.length ? <div className="completion-checklist">{completion.requirements.map((requirement) => <article className={requirement.complete ? "complete" : "pending"} key={requirement.id}>{requirement.complete ? <CheckCircle2 /> : <Clock3 />}<div><b>{requirement.label}</b><span>{requirement.complete ? "Requirement complete" : "Still required"}</span></div></article>)}</div> : null}{passed && completion?.complete && course.certificateEnabled && <section className="certificate-commerce-card"><Award /><div><b>{(completion.certificateFeeGhs ?? course.certificateFeeGhs ?? 0) > 0 ? "Certificate payment required" : "Free verified certificate"}</b><p>{(completion.certificateFeeGhs ?? course.certificateFeeGhs ?? 0) > 0 ? `Your course is complete. Pay GHS ${(completion.certificateFeeGhs ?? course.certificateFeeGhs ?? 0).toLocaleString()} only to generate the UCC QR-verifiable certificate.` : "Your course is complete and this certificate is free."}</p></div><button disabled={certificateBusy} onClick={() => void requestCertificate()}>{certificateBusy ? "Preparing…" : (completion.certificateFeeGhs ?? course.certificateFeeGhs ?? 0) > 0 ? "Pay & generate certificate" : "Generate certificate"}</button></section>}<button className="dialog-primary" onClick={() => setStage(passed ? "materials" : "assessment")}>{passed ? "Return to course activities" : "Try assessment again"}</button></div>}</div>;
  if (stage === "materials") {
    ...
    return <div className="published-course commercial-learner-course"><section className="learner-course-hero"><div><p className="eyebrow">{design.category.toUpperCase()} MICROCREDENTIAL</p><h2>{course.title}</h2><p>{course.description}</p><div><span><Gauge /> {design.level}</span><span><Clock3 /> {design.expectedHours} hours</span><span><BookOpen /> {design.deliveryPattern}</span><span><Users /> {course.facilitatorName}</span></div></div><Award /></section>
      ...
      <div className="certificate-gate-note"><Award /><div><b>UCC digital certificate</b><p>{!course.certificateEnabled ? "This course does not offer a certificate." : (course.certificateFeeGhs ?? 0) > 0 ? `Academic completion is free to record. After completing all requirements, pay GHS ${course.certificateFeeGhs?.toLocaleString()} to generate the scannable UCC verification certificate.` : "Free certificate issued after verified identity, the course assessment and every required programme activity are complete."}</p></div></div>
...
function OpenCourseCatalog({ courses: catalogue, enrolledCodes, query, onEnrol, onPayAndEnrol, onOpen }: { courses: Course[]; enrolledCodes: string[]; query: string; onEnrol: (code: string) => void; onPayAndEnrol: (code: string) => void; onOpen: (course: Course) => void }) {
  ...
  const openCount = available.filter((course) => (course.design?.enrolmentMode ?? "open") === "open").length;
  const takeAction = (course: Course) => {
    const design = course.design ?? defaultCourseDesign();
    if (enrolledCodes.includes(course.code)) return onOpen(course);
    if (design.enrolmentMode !== "open") return toast.info(design.enrolmentMode === "application" ? "Application required" : "Invitation only", { description: "This course does not currently allow self-enrolment." });
    return design.priceGhs > 0 ? onPayAndEnrol(course.code) : onEnrol(course.code);
  };
  return <section className="page-panel open-catalog">
    <div className="page-title"><div><p className="eyebrow">MICROCREDENTIAL CATALOGUE</p><h2>Available courses and programmes</h2><p>Open any course to compare its purpose, outcomes, syllabus, workload and transparent fees before enrolling.</p></div><span className="open-badge"><CheckCircle2 /> {openCount} accepting learners</span></div>
    <div className="discipline-filter" aria-label="Filter programmes by discipline"><button className={selectedDiscipline === "All disciplines" ? "active" : ""} onClick={() => setSelectedDiscipline("All disciplines")}>All disciplines <span>{catalogue.length}</span></button>{presentDisciplines.map((discipline) => <button key={discipline} className={selectedDiscipline === discipline ? "active" : ""} onClick={() => setSelectedDiscipline(discipline)}>{discipline} <span>{catalogue.filter((course) => (course.discipline ?? "Interdisciplinary") === discipline).length}</span></button>)}</div>
    <div className="catalog-grid">{available.map((course) => {
      const enrolled = enrolledCodes.includes(course.code);
      const design = course.design ?? defaultCourseDesign();
      const open = design.enrolmentMode === "open";
      return <article key={course.code}><div className={`catalog-icon ${course.accent}`}><BookOpen /></div><div><span>{course.code}</span><h3>{course.title}</h3><p>{course.school}</p><small><b>{course.discipline ?? "Interdisciplinary"}</b> · {design.level} · {design.expectedHours} hours · {design.deliveryPattern}</small><div className="catalog-fees"><em>{design.priceGhs > 0 ? `Enrolment GHS ${design.priceGhs.toLocaleString()}` : "Free enrolment"}</em><em>{!course.certificateEnabled ? "No certificate" : (course.certificateFeeGhs ?? 0) > 0 ? `Certificate GHS ${course.certificateFeeGhs?.toLocaleString()}` : "Free certificate"}</em></div></div><div className="catalog-card-actions"><button className="catalog-details-button" onClick={() => setDetailsCourse(course)}><Eye /> View details</button><button className={enrolled ? "enrolled" : open ? "" : "restricted-enrolment"} onClick={() => takeAction(course)}>{enrolled ? <><CheckCircle2 /> Open course</> : !open ? <><ShieldCheck /> {design.enrolmentMode === "application" ? "Apply" : "Invitation only"}</> : design.priceGhs > 0 ? <><ShieldCheck /> Pay & enrol</> : <><CirclePlay /> Enrol free</>}</button></div></article>;
    })}{available.length === 0 && <div className="empty-state catalog-empty">No programmes match this discipline and search.</div>}</div>
    <Dialog open={Boolean(detailsCourse)} onOpenChange={(open) => !open && setDetailsCourse(null)}><DialogContent className="course-details-dialog">{detailsCourse && (() => {
      const design = detailsCourse.design ?? defaultCourseDesign();
      const enrolled = enrolledCodes.includes(detailsCourse.code);
      return <><DialogHeader><p className="eyebrow">{detailsCourse.code} · {detailsCourse.discipline ?? "Interdisciplinary"}</p><DialogTitle>{detailsCourse.title}</DialogTitle><DialogDescription>{detailsCourse.facilitatorName ? `Facilitated by ${detailsCourse.facilitatorName}` : "University of Cape Coast"}</DialogDescription></DialogHeader><div className="course-details-body"><p className="course-details-description">{detailsCourse.description}</p><div className="course-detail-facts"><span><Gauge /> {design.level}</span><span><Clock3 /> {design.expectedHours} hours</span><span><BookOpen /> {design.deliveryPattern}</span><span><Users /> {design.enrolmentMode}</span></div><section className="course-fee-disclosure"><div><b>Course access</b><strong>{design.priceGhs > 0 ? `GHS ${design.priceGhs.toLocaleString()}` : "FREE"}</strong><span>{design.priceGhs > 0 ? "Paid once before enrolment" : "No enrolment charge"}</span></div><div><b>Verified certificate</b><strong>{!detailsCourse.certificateEnabled ? "NOT OFFERED" : (detailsCourse.certificateFeeGhs ?? 0) > 0 ? `GHS ${detailsCourse.certificateFeeGhs?.toLocaleString()}` : "FREE"}</strong><span>{(detailsCourse.certificateFeeGhs ?? 0) > 0 ? "Paid only after successful completion" : "Issued after all requirements are met"}</span></div></section><div className="course-detail-columns"><section><h3>What this course accomplishes</h3><ul>{design.objectives.map((objective) => <li key={objective}><CheckCircle2 /> {objective}</li>)}</ul><h3>Who it is for</h3><p>{design.intendedAudience}</p><h3>Prerequisites</h3><p>{design.prerequisites}</p></section><section><h3>Learning outcomes</h3>{design.outcomes.map((outcome, index) => <article key={outcome.id}><span>{index + 1}</span><div><b>{outcome.statement}</b><small>{outcome.skill} · {outcome.assessmentMethod}</small></div></article>)}</section></div><section className="course-syllabus-summary"><h3>Syllabus</h3>{design.sections.map((section, index) => <article key={section.id}><span>{index + 1}</span><div><b>{section.title}</b><p>{section.description}</p></div></article>)}</section><div className="course-detail-footer"><p><ShieldCheck /> Enrolment and certificate charges are separate. Academic completion remains recorded even when a certificate payment is pending.</p><button className="dialog-primary" onClick={() => takeAction(detailsCourse)}>{enrolled ? "Open enrolled course" : design.enrolmentMode !== "open" ? "View admissions route" : design.priceGhs > 0 ? `Pay GHS ${design.priceGhs.toLocaleString()} & enrol` : "Enrol free"}</button></div></div></>;
    })()}</DialogContent></Dialog>
  </section>;
}
...
function OutcomeDesignEditor({ design, onChange }: { design: CourseDesign; onChange: (updater: (current: CourseDesign) => CourseDesign) => void }) {
  ...
  const outcomeGroupsComplete = [
    ...
    design.sections.length >= 1 && sectionComplete === design.sections.length,
  ].filter(Boolean).length;

  return <div className="blueprint-accordion-stack">
    <div className="outcome-design-guide"><span><ShieldCheck /></span><div><b>Outcome design checklist</b><p>Work through the four sections below. Only one opens at a time so the page stays focused and easy to scan.</p></div><em>{outcomeGroupsComplete}/4 sections ready</em></div>
    ...
    <details className="blueprint-accordion section-builder" open={openPanel === "sections"}>
...
function FacilitatorStudio({ email, query, setQuery }: { email: string; query: string, setQuery: (value: string) => void }) {
  ...
  const [certificateFeeGhs, setCertificateFeeGhs] = useState(0);
  ...
  const loadIllustrativeCourse = async () => {
    ...
    try {
      ...
      setGateRequired(example.gateRequired); setQuestionLimit(example.questionLimit); setCertificateEnabled(example.certificateEnabled); setCertificateFeeGhs(0); setPassMark(example.assessmentConfig.passMark); setAttempts(example.assessmentConfig.attempts); setQuestions(example.assessmentConfig.questions); setQuestionFiles(example.assessmentConfig.questionFiles);
  ...
  const loadDraft = (course: StudioDraft) => {
    ...
    setGateRequired(course.gateRequired); setQuestionLimit(course.questionLimit); setCertificateEnabled(course.certificateEnabled); setCertificateFeeGhs(course.certificateFeeGhs ?? 0); setPassMark(course.assessmentConfig?.passMark ?? 70); setAttempts(course.assessmentConfig?.attempts ?? "3"); setQuestions(course.assessmentConfig?.questions ?? []); setQuestionFiles(course.assessmentConfig?.questionFiles ?? []);
  ...
  const buildCourseFromManual = async () => {
    if (!manualFile) return toast.error("Choose a learning manual first.");
    setImportingManual(true);
    try {
      const body = new FormData(); body.append("manual", manualFile);
      const response = await fetch("/api/course-design/from-manual", { method: "POST", body });
      const result = await response.json() as { draft?: { code: string; title: string; discipline: string; description: string; design: CourseDesign; materials: Material[]; activities: CourseActivity[]; assessmentModes: string[]; assessmentConfig: { passMark: number; attempts: string; questions: AssessmentQuestion[]; questionFiles: AssessmentSourceFile[] }; gateRequired: boolean; questionLimit: number; certificateEnabled: boolean; certificateFeeGhs: number }; analysis?: { wordCount?: number; detectedObjectives?: number; detectedOutcomes?: number; detectedSections?: number; note?: string }; error?: string };
      if (!response.ok || !result.draft) throw new Error(result.error ?? "The course draft could not be generated.");
      const draft = result.draft;
      setDraftId(null); setDraftVersion(1); setDraftStatus("new"); setCourseCode(draft.code); setCourseTitle(draft.title); setDiscipline(draft.discipline); setDescription(draft.description); setDesign(draft.design); setMaterials(draft.materials); setCourseActivities(draft.activities); setAssessmentModes(draft.assessmentModes); setPassMark(draft.assessmentConfig.passMark); setAttempts(draft.assessmentConfig.attempts); setQuestions(draft.assessmentConfig.questions); setQuestionFiles(draft.assessmentConfig.questionFiles); setGateRequired(draft.gateRequired); setQuestionLimit(draft.questionLimit); setCertificateEnabled(draft.certificateEnabled); setCertificateFeeGhs(0); setContentSectionId(draft.design.sections[0]?.id ?? "section-1"); setContentOutcomeIds(draft.design.outcomes[0]?.id ? [draft.design.outcomes[0].id] : []); setQuestionOutcomeIds(draft.design.outcomes[0]?.id ? [draft.design.outcomes[0].id] : []); setStep("details"); setIllustrativeTemplateLoaded(false);
      toast.success("Manual converted into a complete editable course draft", { description: `${result.analysis?.wordCount ?? 0} words analysed. Review the generated academic fields before saving.` });
    } catch (error) { toast.error(error instanceof Error ? error.message : "The manual could not be converted."); }
  ...
  const contentIsAccessible = materials.length > 0 && materials.every((material) => material.kind === "Watch" ? Boolean(material.transcriptPublished && material.transcript) : Boolean(material.accessibilityChecked));
  const activityConfigurationValid = courseActivities.every((activity) => Boolean(activity.title.trim() && activity.instructions.trim() && activity.rubric?.trim() && activity.passMark >= 1 && activity.passMark <= 100 && activity.attemptsAllowed >= 1));
  const studioSteps: { id: StudioStepId; number: number; label: string; description: string; guidance: string; checks: { label: string; passed: boolean; detail: string }[] }[] = [
    ...
    },
    {
      id: "outcomes", number: 2, label: "Outcomes", description: "Purpose, evidence and sections", guidance: "Write measurable outcomes, select skills and organise the syllabus.", checks: [
        ...
        { label: "Syllabus sections", passed: design.sections.length >= 1 && design.sections.every((section) => section.title.trim().length >= 3 && section.description.trim().length >= 10), detail: "Give every section a title and clear purpose." },
      ],
    },
    {
      id: "content", number: 3, label: "Content", description: "Lessons, files and alignment", guidance: "Create readable learning blocks and map every outcome to the curriculum.", checks: [
    ...
    {
      id: "activities", number: 4, label: "Activities", description: "Colab and virtual practice", guidance: "Add authentic practice only when the course needs coding or practical evidence.", checks: [
        { label: courseActivities.length ? "Programme activities selected" : "Programme activities considered", passed: true, detail: courseActivities.length ? `${courseActivities.length} programme ${courseActivities.length === 1 ? "activity" : "activities"} added.` : "Optional step: no separate Colab or virtual practical is required." },
        ...
        { label: "Required evidence identified", passed: courseActivities.length === 0 || courseActivities.some((activity) => activity.required) || courseActivities.every((activity) => !activity.required), detail: "Confirm which activities form part of the certificate gate." },
      ],
    },
    {
      id: "assessment", number: 5, label: "Assessment", description: "Questions, feedback and gate", guidance: "Define the scored evidence that proves each learner achieved the outcomes.", checks: [
        { label: "Evidence method selected", passed: assessmentModes.length >= 1, detail: "Select at least one assessment mode." },
        { label: "Scored question authored", passed: questions.slice(0, questionLimit).length >= 1, detail: "Add at least one valid scored question." },
        ...
        { label: "Outcome-linked assessment", passed: questions.length > 0 && questions.every((question) => (question.outcomeIds ?? []).length > 0), detail: "Map every assessment question to one or more outcomes." },
      ],
    },
    {
  ...
  const saveCourse = async (submissionMode: "draft" | "review") => {
    ...
    try {
      const response = await fetch("/api/courses", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id: draftId, expectedVersion: draftVersion, submissionMode, code: courseCode, title: courseTitle, discipline, description, design, materials, activities: courseActivities, assessmentModes, gateRequired, questionLimit, certificateEnabled, certificateFeeGhs, assessmentConfig: { passMark, attempts, questions: questions.slice(0, questionLimit), questionFiles } }),
      });
  ...
  return <div className="authoring-shell">
    <section className="authoring-main page-panel">
      ...
      {step === "outcomes" && <div className="outcome-authoring">
        ...
        <button className="dialog-primary align-right" disabled={!stepComplete(studioSteps[1])} onClick={() => { if (!design.sections.some((item) => item.id === contentSectionId)) setContentSectionId(design.sections[0]?.id ?? "section-1"); setStep("content"); }}>Build learning content <ChevronRight /></button>
      </div>}
      ...
      {step === "assessment" && <div className="assessment-builder">
        ...
        <div className="assessment-publish-settings"><label>Number of questions to set<input type="number" min="1" max="100" value={questionLimit} onChange={(event) => setQuestionLimit(Math.min(100, Math.max(1, Number(event.target.value))))} /><span>{questions.length} of {questionLimit} created</span></label><label className="certificate-setting"><Award /><div><b>Generate UCC QR certificate after completion</b><span>Issued only after verified identity, the course assessment and every activity marked required are complete.</span></div><Switch checked={certificateEnabled} onCheckedChange={(checked) => { setCertificateEnabled(checked); if (!checked) setCertificateFeeGhs(0); }} /></label><label className="certificate-fee-setting"><span>Charge a certificate fee?</span><Switch disabled={!certificateEnabled} checked={certificateEnabled && certificateFeeGhs > 0} onCheckedChange={(checked) => setCertificateFeeGhs(checked ? Math.max(1, certificateFeeGhs || 50) : 0)} />{certificateEnabled && certificateFeeGhs > 0 && <input type="number" min="1" value={certificateFeeGhs} onChange={(event) => setCertificateFeeGhs(Math.max(1, Number(event.target.value)))} />}<small>Default: free. A fee is requested only after academic completion.</small></label></div>
      ...
      {step === "review" && <div className="course-review commercial-quality-review">
        ...
        <section className="studio-fee-review"><div><b>Enrolment</b><strong>{design.priceGhs > 0 ? "GHS " + design.priceGhs.toLocaleString() : "FREE"}</strong><span>{design.priceGhs > 0 ? "Payment before access" : "Self-enrol without payment"}</span></div><div><b>UCC certificate</b><strong>{!certificateEnabled ? "DISABLED" : certificateFeeGhs > 0 ? "GHS " + certificateFeeGhs.toLocaleString() : "FREE"}</strong><span>{certificateFeeGhs > 0 ? "Payment only after completion" : "No certificate charge"}</span></div><p><ShieldCheck /> All courses start free by default. Fees appear only after the facilitator explicitly enables them and remain subject to UCC review.</p></section>
