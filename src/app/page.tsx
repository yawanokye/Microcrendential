...
function PublishedCourseExperience({ course, onOpenActivity, onExit, preview = false }: { course: Course; onOpenActivity: (activity: CourseActivity) => void; onExit?: () => void; preview?: boolean }) {
  ...
  if (stage === "materials") {
    ...
    const sectionComplete = sectionRequiredMaterials.length > 0 && completedInSection === sectionRequiredMaterials.length;
...
function FacilitatorStudio({ email, query, setQuery }: { email: string; query: string, setQuery: (value: string) => void }) {
  ...
  const addQuestion = () => {
    ...
    const question: AssessmentQuestion = { id: editingQuestionId ?? crypto.randomUUID(), type: questionType, prompt: questionPrompt.trim(), options: usesPairs ? [] : options, correctAnswer: selectedAnswer, points: questionPoints, scheme: markingScheme.trim(), feedbackCorrect: feedbackCorrect.trim(), feedbackIncorrect: feedbackIncorrect.trim(), learnerAdvice: learnerAdvice.trim(), outcomeIds: questionOutcomeIds, pairs: usesPairs ? pairs : undefined, videoUrl: questionType === "Video question" ? questionVideoUrl.trim() : undefined, videoMode: questionType === "Video question" ? videoMode : undefined, videoStart: questionType === "Video question" ? videoStart : undefined, videoEnd: questionType === "Video question" ? videoEnd : undefined, whiteboardEnabled, previewed: false, approved: false };
