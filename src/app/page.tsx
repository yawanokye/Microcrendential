...
function PublishedCourseExperience({ course, onOpenActivity, onExit, preview = false }: { course: Course; onOpenActivity: (activity: CourseActivity) => void; onExit?: () => void; preview?: boolean }) {
  ...
  if (stage === "materials") {
    ...
    const sectionStandaloneActivities=sectionActivities.filter((activity)=>activity.kind!=="inline");
...
function FacilitatorStudio({ email, query, setQuery }: { email: string; query: string, setQuery: (value: string) => void }) {
  ...
  const studioSteps: { id: StudioStepId; number: number; label: string; description: string; guidance: string; checks: { label: string; passed: boolean; detail: string }[] }[] = [
    ...
    {
      id: "activities", number: 4, label: "Activities", description: isBroaderCredential ? "Inherited component activities" : "Colab and virtual practice", guidance: isBroaderCredential ? "The broader credential inherits required learning activities from its component credentials. Add no duplicate activities." : "Add authentic practice only when the course needs coding or practical evidence.", checks: isBroaderCredential ? [
  ...
  const saveMaterialEditor = () => {
    ...
    setMaterialEdit(null); setMaterialActivityEdit(null); setMaterialActivityEnabled(false);
  ...
  return <div className="authoring-shell">
    <section className="authoring-main page-panel">
      ...
      {step === "activities" && (isBroaderCredential ? <div className="broader-inherited-stage"><div className="studio-section-heading"><div><p className="eyebrow">INHERITED LEARNING ACTIVITIES</p><h3>Completion evidence remains with each component</h3><p>A learner must satisfy every required activity and feedback rule inside the component microcredentials. The broader credential does not duplicate or re-mark those activities.</p></div><CheckCircle2 /></div><div className="inherited-evidence-grid">{selectedComponentCourses.map((component) => { const requiredActivities = component.activities.filter((activity) => activity.required); return <article key={component.id}><header><div><small>{component.code}</small><h3>{component.title}</h3></div><b>{requiredActivities.length} required</b></header>{requiredActivities.length ? <div>{requiredActivities.map((activity) => <p key={activity.id}><CheckCircle2 /><span><b>{activity.title}</b><small>Pass {activity.passMark}% · {activity.gradingMode === "ai_luna" ? "Luna" : activity.gradingMode === "ai_terra" ? "Terra" : activity.gradingMode === "ai_auto" ? "Luna/Terra" : activity.gradingMode === "rule" ? "Rule based" : "Facilitator"} grading · feedback required before completion</small></span></p>)}</div> : <p className="muted-evidence">No separate required activity is configured for this component.</p>}</article>; })}</div><button className="dialog-primary align-right" disabled={!stepComplete(studioSteps[3])} onClick={() => setStep("assessment")}>Review inherited assessments <ChevronRight /></button></div> : <div className="programme-activities-builder">
