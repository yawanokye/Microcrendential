...
type CertificateRecord = { certificate_code: string; learner_name: string; course_code: string; course_title: string; credential_type?: string; status?: string; issued_at: string; expires_at?: string | null; revoked_at?: string | null; revocation_reason?: string | null; sharePath?: string };
...
function CertificateCard({ certificate }: { certificate: CertificateRecord }) {
  ...
  return <article className={`certificate-card ${active ? "" : "revoked"}`}><div className="certificate-seal"><GraduationCap /><span>UNIVERSITY OF CAPE COAST</span></div><span className={`credential-state ${active ? "active" : "revoked"}`}><ShieldCheck /> {active ? "Active credential" : "Revoked credential"}</span><p className="eyebrow">DIGITAL MICROCREDENTIAL CERTIFICATE</p><h2>Certificate of Completion</h2><p>This certifies that</p><h3>{certificate.learner_name}</h3><p>has successfully completed all required learning activities and assessments for</p><h4>{certificate.course_title}</h4><div className="certificate-meta"><span><b>Course code</b>{certificate.course_code}</span><span><b>Issued</b>{new Date(certificate.issued_at).toLocaleDateString()}</span><span><b>Verification</b>{certificate.certificate_code}</span></div>{!active && <div className="credential-revocation"><ShieldCheck /><div><b>This credential is not currently valid</b><span>{certificate.revocation_reason || "Contact the issuing institution for details."}</span></div></div>}<div className="certificate-verify"><VerificationQr code={certificate.certificate_code} /><div><QrCode /><b>Public verification record</b><span>{certificate.certificate_code}</span><a href={verificationPath} target="_blank">Verify online</a></div></div><div className="certificate-actions"><button className="secondary-action certificate-print" onClick={() => window.print()}><Award /> Print certificate</button><button className="secondary-action" onClick={copyVerification}><QrCode /> Copy verification link</button></div></article>;
}
...
function PublishedCourseExperience({ course, onOpenActivity }: { course: Course; onOpenActivity: (activity: CourseActivity) => void }) {
