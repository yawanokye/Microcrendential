...
function VideoTranscriptDialog({ material, onClose, onComplete, completeLabel = "Complete and continue" }: { material: CourseMaterial | null; onClose: () => void; onComplete?: () => void; completeLabel?: string }) {
...
function PublishedCourseExperience({ course, onOpenActivity, onExit, preview = false }: { course: Course; onOpenActivity: (activity: CourseActivity) => void; onExit?: () => void; preview?: boolean }) {
  ...
  const questions = course.assessmentConfig?.questions ?? []; const orderedMaterials = course.materials ?? []; const design = course.design ?? defaultCourseDesign(); const sections = design.sections.length ? design.sections : [{ id: "section-1", title: "Course learning", description: "Guided microcredential content" }]; const [stage, setStage] = useState<"materials" | "assessment" | "result">("materials"); const [activeSectionIndex, setActiveSectionIndex] = useState(0); const [answers, setAnswers] = useState<Record<string, unknown>>({}); const [videoReady, setVideoReady] = useState<Record<string, boolean>>({}); const [score, setScore] = useState<number | null>(null); const [passed, setPassed] = useState(false); const [awaitingMarking, setAwaitingMarking] = useState(false); const [certificate, setCertificate] = useState<CertificateRecord | null>(null); const [completion, setCompletion] = useState<Completion | null>(null); const [submitting, setSubmitting] = useState(false); const [selectedMaterial, setSelectedMaterial] = useState<CourseMaterial | null>(null); const [completedMaterialIds, setCompletedMaterialIds] = useState<string[]>([]); const [progressLoading, setProgressLoading] = useState(!preview); const sectionViewRef = useRef<HTMLDivElement | null>(null);
  ...
  const requiredMaterials = orderedMaterials.filter((material) => material.required !== false); const requiredComplete = requiredMaterials.every((material) => completedMaterialIds.includes(materialId(material, orderedMaterials.indexOf(material)))); const learningPercent = requiredMaterials.length ? Math.round((requiredMaterials.filter((material) => completedMaterialIds.includes(materialId(material, orderedMaterials.indexOf(material)))).length / requiredMaterials.length) * 100) : 100;
  ...
  if (stage === "materials") {
    ...
    const completedInSection = sectionMaterials.filter((material) => completedMaterialIds.includes(materialId(material, orderedMaterials.indexOf(material)))).length;
    const sectionComplete = sectionMaterials.length > 0 && completedInSection === sectionMaterials.length;
    ...
    return <div className="published-course commercial-learner-course">
      ...
      <button className="dialog-primary" disabled={!questions.length || !requiredComplete} title={!requiredComplete ? "Complete every required lesson first." : undefined} onClick={() => setStage("assessment")}><FileCheck2 /> {!requiredComplete ? "Complete required lessons to unlock assessment" : questions.length ? `Begin ${questions.length}-question assessment` : "Assessment awaiting facilitator"}</button><VideoTranscriptDialog material={selectedMaterial} onClose={() => setSelectedMaterial(null)} onComplete={completeLesson} completeLabel={orderedMaterials.indexOf(selectedMaterial!) < orderedMaterials.length - 1 ? "Complete and continue" : "Complete final lesson"} />
    </div>;
  }
...
function CourseApprovalPanel() {
  ...
  const review = async (course: CourseReviewRecord, status: "active" | "rejected", quality: ReturnType<typeof evaluateCourseQuality>) => {
    ...
    if (status === "active" && !quality.ready) return toast.error("Publication is blocked until every quality check is complete.");
    setSavingId(course.id);
    try {
      const response = await fetch("/api/courses", {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id: course.id, status, comment }),
      });
  ...
  return <section className="page-panel course-approval-panel"><div className="page-title"><div><p className="eyebrow">ACADEMIC ACTIVATION</p><h2>Facilitator course approvals</h2><p>Review the learner experience, outcomes, accessibility and assessment. Publication is permitted only when every quality check passes.</p></div><span className="access-badge"><BookOpen /> {pendingCount} pending{returnedCount ? ` · ${returnedCount} returned` : ""}</span></div>{loading && <div className="empty-state">Loading facilitator courses…</div>}<div className="course-approval-list commercial-approval-list">{items.map((course) => {
    ...
    const actionable = course.status === "pending_review" && quality.ready;
    const requiresCorrection = course.status === "pending_review" && !quality.ready;
    return <article key={course.id}><div><span>{course.code} · {course.discipline} · version {course.versionNumber} · {course.status.replaceAll("_", " ")}</span><h3>{course.title}</h3><p>{course.facilitatorName} · {course.design?.outcomes?.length ?? 0} outcomes · {course.materials?.length ?? 0} learning blocks · {course.activities?.length ?? 0} applied activities</p><div className="admin-quality-meter"><Progress value={quality.score} /><b>{quality.score}% quality readiness</b></div><div className="admin-course-facts"><span>{course.design?.expectedHours ?? 0} hours</span><span>{course.design?.level ?? "level not set"}</span><span>{course.certificateEnabled ? "UCC QR certificate" : "certificate disabled"}</span></div>{course.reviewComment && course.status !== "pending_review" && <p className="admin-existing-comment"><MessageSquareText /> {course.reviewComment}</p>}{requiresCorrection && <div className="approval-exception-note"><AlertTriangle /><div><b>Corrections required before publication</b><span>Outstanding checks: {missingChecks.map((check) => check.label).join(", ")}. Return the course with specific guidance for the facilitator.</span></div></div>}</div><div className="approval-decision"><label>Review comments<textarea value={comments[course.id] ?? ""} onChange={(event) => setComments((current) => ({ ...current, [course.id]: event.target.value }))} placeholder="Required when returning; optional approval note when publishing." /></label><div><button className="reject" disabled={course.status !== "pending_review" || savingId === course.id} onClick={() => review(course, "rejected", quality)}>Return with comments</button><button className="approve" disabled={!actionable || savingId === course.id} title={!actionable ? "Publication requires a pending review and 100% readiness." : "Approve and publish this course."} onClick={() => review(course, "active", quality)}><CheckCircle2 /> {savingId === course.id ? "Saving…" : "Approve & publish"}</button></div></div></article>;
  })}{!loading && items.length === 0 && <div className="empty-state">No facilitator courses have been submitted.</div>}</div></section>;
}

type RevisionReviewRecord = CourseReviewRecord & { id: number; courseId: number; revisionNumber: number; status: string; reviewComment?: string | null };

function RevisionApprovalPanel() {
  const [items, setItems] = useState<RevisionReviewRecord[]>([]); const [comments, setComments] = useState<Record<number, string>>({}); const [saving, setSaving] = useState<number | null>(null);
  const load = useCallback(async () => { const response = await fetch("/api/courses/revisions"); const result = await response.json() as { revisions?: RevisionReviewRecord[]; error?: string }; if (!response.ok) return toast.error(result.error ?? "Course revisions could not be loaded."); setItems((result.revisions ?? []).filter((item) => item.status === "pending_review")); }, []);
  useEffect(() => { void load(); }, [load]);
  const decide = async (revision: RevisionReviewRecord, status: "applied" | "rejected") => { const comment = comments[revision.id]?.trim() ?? ""; if (status === "rejected" && !comment) return toast.error("Add comments explaining the required revision."); setSaving(revision.id); try { const response = await fetch("/api/courses/revisions", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ revisionId: revision.id, status, comment }) }); const result = await response.json() as { error?: string }; if (!response.ok) throw new Error(result.error ?? "The revision decision could not be saved."); toast.success(status === "applied" ? "Revision approved and applied" : "Revision returned to facilitator", { description: status === "applied" ? "Learners now see the approved version." : undefined }); await load(); } catch (error) { toast.error(error instanceof Error ? error.message : "The revision decision could not be saved."); } finally { setSaving(null); } };
  return <section className="page-panel revision-approval-panel"><div className="page-title"><div><p className="eyebrow">CONTROLLED CHANGE</p><h2>Published-course revisions</h2><p>The existing student version remains live until a complete revision passes review.</p></div><span className="access-badge"><Pencil /> {items.length} pending</span></div><div className="revision-review-list">{items.map((revision) => { const quality = evaluateCourseQuality({ title: revision.title, description: revision.description, design: revision.design ?? defaultCourseDesign(), materials: revision.materials ?? [], activities: revision.activities, assessmentConfig: revision.assessmentConfig, questionCount: revision.assessmentConfig?.questions?.length ?? 0 }); return <article key={revision.id}><header><div><span>{revision.code} · revision {revision.revisionNumber}</span><h3>{revision.title}</h3><p>{revision.facilitatorName} · {quality.score}% quality readiness</p></div><span className={quality.ready ? "ready" : "blocked"}>{quality.ready ? <CheckCircle2 /> : <AlertTriangle />}{quality.ready ? "Ready" : "Blocked"}</span></header><div className="revision-checks">{quality.checks.map((check) => <span className={check.passed ? "complete" : "pending"} key={check.id}>{check.passed ? <CheckCircle2 /> : <Clock3 />}{check.label}</span>)}</div><label>Decision comments<textarea value={comments[revision.id] ?? ""} onChange={(event) => setComments((current) => ({ ...current, [revision.id]: event.target.value }))} placeholder="Explain any required correction, or add an optional approval note." /></label><footer><button className="reject" disabled={saving === revision.id} onClick={() => void decide(revision, "rejected")}>Return revision</button><button className="approve" disabled={!quality.ready || saving === revision.id} onClick={() => void decide(revision, "applied")}><CheckCircle2 /> Approve and apply</button></footer></article>; })}{items.length === 0 && <div className="empty-state">No published-course revisions are waiting for review.</div>}</div></section>;
}
...
function SectionContentWorkspace({ section, number, outcomes, materials, onAdded, onRemove }: {
  ...
  const [videoTitle, setVideoTitle] = useState(""); const [videoUrl, setVideoUrl] = useState(""); const [videoTranscript, setVideoTranscript] = useState("");
  ...
  const uploadInlinePicture = async (image: File): Promise<void> => {
    ...
    try {
      ...
      insertEditorHtml(`<figure><img src="${source}" alt="${safeAlt}"><figcaption>${safeAlt}</figcaption></figure>`); setInlineAssetKeys((items) => [...new Set([...items, result.key!])]);
...
function FacilitatorStudio({ email, query, setQuery }: { email: string; query: string, setQuery: (value: string) => void }) {
  ...
  const createCourseFromManual = async () => {
    ...
    try {
      ...
      setManualImportInfo(`${result.extraction?.fileName ?? manualFile.name} · ${(result.extraction?.wordCount ?? 0).toLocaleString()} words extracted. ${result.warning ?? "Review every generated field before submission."}`);
  ...
  const loadDraft = async (course: StudioDraft) => {
    ...
    if (course.status === "active") {
      ...
      if (!response.ok || !result.revision) return toast.error(result.error ?? "A controlled revision could not be opened.");
      if (result.revision.status === "pending_review") return toast.info("This revision is awaiting approval", { description: "The published course remains active. Editing will reopen after the administrator returns or applies the revision." });
      editable = { ...course, ...result.revision, id: course.id, versionNumber: result.revision.revisionNumber, status: result.revision.status };
      setRevisionId(result.revision.id);
      toast.success(`Revision ${result.revision.revisionNumber} opened`, { description: "The live course stays unchanged until this revision is approved." });
  ...
  const quality = evaluateCourseQuality({ title: courseTitle, description, design, materials, activities: courseActivities, assessmentConfig: { passMark, attempts, questions: questions.slice(0, questionLimit) }, questionCount: questions.slice(0, questionLimit).length });
  ...
  const saveCourse = async (submissionMode: "draft" | "review") => {
    ...
    try {
      const response = await fetch(revisionId ? "/api/courses/revisions" : "/api/courses", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id: draftId, revisionId, expectedVersion: draftVersion, submissionMode, code: courseCode, title: courseTitle, discipline, description, design, materials, activities: courseActivities, assessmentModes, gateRequired, questionLimit, certificateEnabled, assessmentConfig: { passMark, attempts, questions: questions.slice(0, questionLimit), questionFiles } }),
      });
      ...
      await refreshDrafts();
      toast.success(submissionMode === "review" ? "Submitted for academic review" : revisionId ? "Revision saved" : "Draft saved", { description: submissionMode === "review" ? `${courseTitle} is now locked into the UCC quality-review queue. The currently published course remains live until approval.` : `Version ${result.course?.versionNumber ?? result.revision?.revisionNumber ?? draftVersion} is safely stored.` });
    } catch (error) {
  ...
  return <div className="authoring-shell">
    <section className="authoring-main page-panel">
      ...
      {authoringMethod === "ai" ? <AiCourseDesigner currentDraft={{ title: courseTitle, code: courseCode, discipline, description, design, materials, activities: courseActivities, assessmentModes, assessmentConfig: { passMark, attempts, questions }, certificateEnabled }} onApprove={applyAiProposal} onUseManual={() => setAuthoringMethod("manual")} /> : <section className="manual-import-card">
        ...
        <div><p className="eyebrow">MANUAL DESIGN OR IMPORT</p><h2>Build every stage yourself or extract an editable starting draft</h2><p>Use the blank guided Studio for full manual control, or import an accessible PDF, DOCX, TXT, Markdown, HTML or RTF manual. The rule-based importer fills the blueprint, sections, learning blocks and draft assessments without calling AI.</p><div className="manual-import-notes"><span><CheckCircle2 /> All six design stages</span><span><CheckCircle2 /> Self-enrolment and free by default</span><span><ShieldCheck /> Facilitator review remains compulsory</span></div></div>
        <div className="manual-import-actions"><button className="secondary-action" type="button" onClick={startNewCourse}><BookOpen /> Start a blank guided course</button><label><Upload /><b>{manualFile?.name ?? "Or choose a course manual"}</b><small>Supported document formats · maximum 25 MB</small><input type="file" accept=".pdf,.docx,.txt,.md,.html,.htm,.rtf" onChange={(event) => { setManualFile(event.target.files?.[0] ?? null); setManualImportInfo(""); }} /></label><button className="dialog-primary" disabled={!manualFile || importingManual} onClick={() => void createCourseFromManual()}>{importingManual ? "Extracting all course stages…" : "Import without AI"}</button></div>
      ...
      {step === "content" && <div className="content-authoring commercial-content-authoring">
        <section className="section-content-builder"><div className="studio-section-heading"><div><p className="eyebrow">SECTION-BY-SECTION COURSE BUILDING</p><h3>Add the learning content beneath each approved section</h3><p>Open a section, then upload a document, paste a rich lesson with diagrams or pictures, or embed a teaching video. Every item is automatically placed in that section for the student reader.</p></div><BookOpen /></div><div className="section-content-workspace-list">{design.sections.map((section, index) => <SectionContentWorkspace key={section.id} section={section} number={index + 1} outcomes={design.outcomes} materials={materials.filter((material) => material.sectionId === section.id)} onAdded={(material) => setMaterials((items) => [...items, material])} onRemove={(material) => setMaterials((items) => items.filter((item) => item !== material))} />)}</div></section>
