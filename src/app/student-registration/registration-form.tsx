...
import {
  ArrowLeft, ArrowRight, BookOpen, Camera, CheckCircle2, FileText,
  LockKeyhole, ShieldCheck, Sparkles, Upload, UserRound, Video,
} from "lucide-react";
import { UccBrandLockup } from "@/components/ucc-brand";

type Stage = "loading" | "account" | "verify" | "profile" | "identity" | "review" | "pending" | "existing";
...
function RegistrationSelfie({ ready, uploading, onCaptured }: { ready: boolean; uploading: boolean; onCaptured: (file: File) => Promise<void> }) {
  ...
  const capture = () => {
    ...
    canvas.toBlob((blob) => {
      ...
      const captured = new File([blob], `ucc-learner-selfie-${Date.now()}.jpg`, { type: "image/jpeg" });
...
export default function StudentRegistrationForm() {
  ...
  const [challengeId, setChallengeId] = useState(""); const [verificationCode, setVerificationCode] = useState("");
  ...
  const createAccount = async (event: React.FormEvent) => {
    ...
    if (!account.terms) return setError("Accept the learner-account terms to continue.");
    setSaving(true);
    try {
      ...
      const result = await response.json() as { error?: string; challengeId?: string; challengeRequired?: string };
      if (!response.ok) throw new Error(result.error ?? "The learner account could not be created.");
      setIdentity({ email: account.email.trim().toLowerCase(), fullName: account.fullName.trim() });
      if (result.challengeRequired && result.challengeId) { setChallengeId(result.challengeId); setStage("verify"); }
      else setStage("profile");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "The learner account could not be created."); }
  ...
  const verifyEmail = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/render-auth", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ mode: "verify_challenge", portal: "learner", challengeId, code: verificationCode }) });
      const result = await response.json() as { error?: string };
      if (!response.ok) throw new Error(result.error ?? "The email code could not be verified.");
      setStage("profile");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "The email code could not be verified."); }
    finally { setSaving(false); }
  };
  ...
  const validateProfile = () => {
    ...
    if (required.some((value) => !value.trim())) return setError("Complete every required learner-profile field.");
