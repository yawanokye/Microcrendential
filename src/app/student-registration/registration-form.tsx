...
type UploadedEvidence = { name: string; size: number; type: string };
...
export default function StudentRegistrationForm() {
  ...
  const [idDocument, setIdDocument] = useState<UploadedEvidence | null>(null);
  ...
  const createAccount = async (event: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const response = await fetch("/api/render-auth", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ mode: "register", portal: "learner", fullName: account.fullName, email: account.email, password: account.password, termsAccepted: account.terms, returnTo: "/student-registration" }) });
  ...
  const upload = async (file: File, kind: "id" | "selfie") => {
    ...
    const acceptedExtension = kind === "id" ? ["jpg", "jpeg", "png", "pdf"].includes(extension) : ["jpg", "jpeg", "png"].includes(extension);
    const acceptedType = kind === "id" ? ["image/jpeg", "image/png", "application/pdf"].includes(file.type) : ["image/jpeg", "image/png"].includes(file.type);
    if (!acceptedExtension && !acceptedType) return setError(kind === "id" ? "Choose a JPG, PNG or PDF identity scan." : "Choose a JPG or PNG live photo.");
    if (!file.size) return setError("The selected file is empty. Export or scan the document again.");
    if (file.size > 8 * 1024 * 1024) return setError("Identity files must be 8 MB or smaller.");
    const previousKey = idDocumentKey;
    const previousDocument = idDocument;
    if (kind === "id") setIdDocument({ name: file.name, size: file.size, type: file.type || extension.toUpperCase() });
    setUploading(kind); setError("");
    try {
      const body = new FormData(); body.append("file", file); body.append("kind", kind === "id" ? "national-id" : "selfie");
      const response = await fetch("/api/identity-uploads", { method: "POST", body }); const result = await response.json() as { key?: string; name?: string; size?: number; type?: string; error?: string };
      if (!response.ok || !result.key) throw new Error(result.error ?? "The identity file could not be secured.");
      if (kind === "id") {
        setIdDocumentKey(result.key);
        setIdDocument({ name: result.name ?? file.name, size: result.size ?? file.size, type: result.type ?? file.type });
        if (previousKey && previousKey !== result.key) {
          void fetch("/api/identity-uploads", { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ key: previousKey }) });
        }
      } else setSelfieKey(result.key);
    } catch (reason) {
      if (kind === "id") setIdDocument(previousDocument);
      setError(reason instanceof Error ? reason.message : "The identity file could not be secured.");
    }
    finally { setUploading(null); }
  };
  const chooseIdDocument = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.currentTarget.files?.[0];
    event.currentTarget.value = "";
    if (file) void upload(file, "id");
  };
  const removeIdDocument = async () => {
    if (!idDocumentKey) return;
    setUploading("id"); setError("");
    try {
      const response = await fetch("/api/identity-uploads", { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ key: idDocumentKey }) });
      if (!response.ok && response.status !== 404) throw new Error("The attached scan could not be removed.");
      setIdDocumentKey(""); setIdDocument(null);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "The attached scan could not be removed."); }
    finally { setUploading(null); }
  };
  ...
  if (stage === "existing") return <main className="student-registration-shell"><section className="registration-complete"><div className="registration-brand"><GraduationCap /><div><b>UCC Microcredentials</b><span>Student Registration Portal</span></div></div><div className="complete-mark active"><CheckCircle2 /></div><p className="eyebrow">ACCOUNT ALREADY REGISTERED</p><h1>Your UCC student profile is already available.</h1><p>Continue to the Student Portal to access learning, assessments, your skills passport and verifiable credentials.</p>{studentNumber && <div className="student-number"><span>Student number</span><b>{studentNumber}</b></div>}<a className="registration-primary" href="/?portal=learner">Open Student Portal <ArrowRight /></a></section></main>;
