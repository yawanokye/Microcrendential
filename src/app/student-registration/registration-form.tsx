...
export default function StudentRegistrationForm() {
  ...
  const createAccount = async (event: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const response = await fetch("/api/render-auth", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ mode: "register", fullName: account.fullName, email: account.email, password: account.password, termsAccepted: account.terms, returnTo: "/student-registration" }) });
